...
async fn enable_feature_0() {
    ...
    let randomness_effective_epoch = get_current_epoch(&client).await;
    info!(
        "Randomness main logic became effective in epoch {}.",
        randomness_effective_epoch
    );

    info!("Enabling validator transactions.");
    ...
    let txn_summary = cli
        .run_script_with_gas_options(root_idx, enable_vtxn_script.as_str(), Some(governance_gas))
        .await
        .expect("Txn execution error.");
    ...
    let previous_target_epoch = None;
