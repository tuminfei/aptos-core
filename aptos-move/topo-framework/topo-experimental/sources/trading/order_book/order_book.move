module poto_experimental::order_book {
    friend poto_experimental::order_placement;
    friend poto_experimental::order_operations;
    friend poto_experimental::market_types;
    friend poto_experimental::market_bulk_order;
    friend poto_experimental::dead_mans_switch_operations;
    #[test_only]
    friend poto_experimental::order_book_client_order_id;

    use std::option::Option;
    use std::string::String;
    use poto_trading::order_book_types::{OrderId, TriggerCondition};
    use poto_trading::bulk_order_types::{
        BulkOrder,
        BulkOrderRequest,
        BulkOrderPlaceResponse
    };
    use poto_trading::order_match_types::{OrderMatch, OrderMatchDetails};
    use poto_trading::single_order_types::{SingleOrder, SingleOrderRequest};
    use poto_experimental::bulk_order_book::{BulkOrderBook, new_bulk_order_book};
    use poto_experimental::single_order_book::{SingleOrderBook, new_single_order_book};
    use poto_experimental::price_time_index::{PriceTimeIndex, new_price_time_idx};
    ...
    public fun set_up_test_with_id(): OrderBook<u64> {
        poto_framework::timestamp::set_time_has_started_for_testing(
            &poto_framework::account::create_signer_for_test(@0x1)
        );
        new_order_book()
    }
}
