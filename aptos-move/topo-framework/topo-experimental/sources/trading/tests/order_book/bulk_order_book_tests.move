#[test_only]
module poto_experimental::bulk_order_book_tests {
    use std::vector;
    use poto_framework::timestamp;
    use poto_framework::account;
    use poto_trading::order_match_types::OrderMatch;
    use poto_trading::bulk_order_types::BulkOrderRequest;
    use poto_experimental::bulk_order_utils::new_bulk_order_request_with_sanitization;
    use poto_experimental::bulk_order_book::{BulkOrderBook, new_bulk_order_book};
    use poto_experimental::price_time_index;

    #[test_only]
    use poto_trading::order_book_types::{TestMetadata, new_test_metadata};
    ...
    fun setup_test(): (BulkOrderBook<TestMetadata>, price_time_index::PriceTimeIndex) {
        let poto_framework = account::create_signer_for_test(@0x1);
        ...
        let order_request =
            create_test_order_request_with_sequence(
                TEST_ACCOUNT_1,
    ...
    fun place_taker_order_and_get_matches(
        ...
        while (remaining_size > 0) {
            ...
            let result =
                price_time_index.get_single_match_result(
    ...
    fun create_order_data(
        ...
        OrderData { account, bid_price, bid_size, ask_price, ask_size }
    }
    ...
    fun test_basic_matching_for_taker_bid() {
        ...
        let matches =
            place_taker_order_and_get_matches(
                &mut order_book,
    ...
    #[test]
    #[expected_failure(abort_code = poto_experimental::bulk_order_book::EORDER_NOT_FOUND)]
    ...
    #[test]
    #[expected_failure(abort_code = poto_experimental::bulk_order_utils::E_BID_ORDER_INVALID)]
    ...
    #[test]
    #[expected_failure(abort_code = poto_experimental::bulk_order_utils::E_ASK_ORDER_INVALID)]
    ...
    #[test]
    #[expected_failure(abort_code = poto_experimental::bulk_order_utils::EPRICE_CROSSING)]
    ...
    fun test_spread_crossing_prevented_by_active_order_book() {
        ...
        let response2 = order_book.place_bulk_order(
            &mut price_time_index, order_request_2
        );
        assert!(response2.is_success_response());

        let (
            _order2,
            cancelled_bid_prices2,
            cancelled_bid_sizes2,
            cancelled_ask_prices2,
            cancelled_ask_sizes2,
            _previous_seq_num_option2
        ) = response2.destroy_bulk_order_place_response_success();
    ...
    fun test_two_accounts_same_price_level() {
        ...
        setup_multi_account_scenario(
            &mut order_book, &mut price_time_index, accounts_and_orders
        );
    ...
    fun test_order_replacement_multiple_accounts() {
        ...
        place_simple_order_with_sequence(
            &mut order_book,
            &mut price_time_index,
            TEST_ACCOUNT_1,
            30,
    ...
    fun test_sequence_number_validation() {
        ...
        let (
            _order1,
            _cancelled_bid_prices1,
            _cancelled_bid_sizes1,
            _cancelled_ask_prices1,
            _cancelled_ask_sizes1,
            previous_seq_num_option1
        ) = response1.destroy_bulk_order_place_response_success();
        ...
        assert!(bulk_order.get_order_request().get_sequence_number() == 10);
        ...
        let (
            _order2,
            _cancelled_bid_prices2,
            _cancelled_bid_sizes2,
            _cancelled_ask_prices2,
            _cancelled_ask_sizes2,
            previous_seq_num_option2
        ) = response2.destroy_bulk_order_place_response_success();
        ...
        let (rejected_account, rejected_seq_num, existing_seq_num) =
            response3.destroy_bulk_order_place_response_rejection();
    ...
    fun test_cancel_bulk_order_at_price() {
        ...
        let (cancelled_size, _) =
            order_book.cancel_bulk_order_at_price(
                &mut price_time_index,
                TEST_ACCOUNT_1,
                BID_PRICE_1,
                true
