#[test_only]
module poto_experimental::dead_mans_switch_tracker_test {
    use std::option;
    use std::signer;
    ...
    fun setup_test(poto_framework: &signer): DeadMansSwitchTracker {
    ...
    public fun test_new_tracker_allows_all_orders(
        ...
        let tracker = setup_test(poto_framework);
        let user_addr = @0x123;
    ...
    #[test(poto_framework = @0x1, user = @0x123)]
    public fun test_keep_alive_update(
        poto_framework: &signer, user: &signer
    ) {
        let tracker = setup_test(poto_framework);
    ...
    #[test(poto_framework = @0x1, user1 = @0x123, user2 = @0x456)]
    public fun test_multiple_users_independent(
        poto_framework: &signer, user1: &signer, user2: &signer
    ) {
        let tracker = setup_test(poto_framework);
    ...
    #[
        expected_failure(
            abort_code = 0, location = poto_experimental::dead_mans_switch_tracker
        )
    ]
