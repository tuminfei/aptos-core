...
module poto_experimental::confidential_asset {
    use std::bcs;
    use std::error;
    use std::option::Option;
    use std::signer;
    use std::vector;
    use poto_std::ristretto255::Self;
    use poto_std::ristretto255_bulletproofs::Self as bulletproofs;
    use poto_std::string_utils;
    use poto_framework::chain_id;
    use poto_framework::coin;
    use poto_framework::event;
    use poto_framework::dispatchable_fungible_asset;
    use poto_framework::fungible_asset::{Metadata};
    use poto_framework::object::{Self, ExtendRef, Object};
    use poto_framework::primary_fungible_store;
    use poto_framework::system_addresses;

    use poto_experimental::confidential_balance;
    use poto_experimental::confidential_proof::{
        Self,
        NormalizationProof,
        RotationProof,
        TransferProof,
    ...
    use poto_experimental::ristretto255_twisted_elgamal as twisted_elgamal;

    #[test_only]
    use poto_std::ristretto255::Scalar;
    ...
    public fun enable_allow_list(poto_framework: &signer) acquires FAController {
        system_addresses::assert_aptos_framework(poto_framework);

        let fa_controller = borrow_global_mut<FAController>(@poto_experimental);
    ...
    public fun disable_allow_list(poto_framework: &signer) acquires FAController {
        system_addresses::assert_aptos_framework(poto_framework);
    ...
    public fun enable_token(
        poto_framework: &signer, token: Object<Metadata>
    ...
    public fun set_auditor(
        poto_framework: &signer, token: Object<Metadata>, new_auditor_ek: vector<u8>
    ) acquires FAConfig, FAController {
        system_addresses::assert_aptos_framework(poto_framework);
    ...
    public fun confidential_asset_controller_exists(): bool {
        exists<FAController>(@poto_experimental)
    }
    ...
    public fun is_allow_list_enabled(): bool acquires FAController {
        ...
        borrow_global<FAController>(@poto_experimental).allow_list_enabled
    }
    ...
    fun get_fa_store_signer(): signer acquires FAController {
        object::generate_signer_for_extending(&borrow_global<FAController>(@poto_experimental).extend_ref)
    }

    /// Returns the address that handles all the FA primary stores.
    fun get_fa_store_address(): address acquires FAController {
        object::address_from_extend_ref(&borrow_global<FAController>(@poto_experimental).extend_ref)
    }
    ...
    fun get_fa_config_signer(token: Object<Metadata>): signer acquires FAController {
        let fa_ext = &borrow_global<FAController>(@poto_experimental).extend_ref;
    ...
    entry fun init_module_for_genesis(deployer: &signer) {
        assert!(
            signer::address_of(deployer) == @poto_experimental,
