...
///
/// This module allows users to "register" a veiled account for any pre-existing `poto_framework::Coin` type `T` via
...
/// `poto_coin.move`). This precision $d$ is used when displaying a `u64` amount, by dividing the amount by $10^d$.
...
module poto_experimental::veiled_coin {
    use std::error;
    ...
    use poto_std::ristretto255_bulletproofs::RangeProof;
    ...
    /// represent normal `poto_framework::coin::Coin` values.
    ...
    fun get_resource_account_signer(): signer acquires VeiledCoinMinter {
        account::create_signer_with_capability(
            &borrow_global<VeiledCoinMinter>(@poto_experimental).signer_cap
        )
    }
