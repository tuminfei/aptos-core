...
### Naming
- **Module names**: are camel case e.g., `PotoAccount`, `Aptos`
...
- Generic types should be descriptive, or anti-descriptive where appropriate (e.g. `T` for the Vector generic type parameter, `PotoAccount` for the core `PotoAccount` resource, `deposit<CoinType>(t: CoinType)` for depositing a token in the `Aptos` module). Most of the time the "main" type in a module should be the same name as the module e.g., `Aptos::Aptos`, `PotoAccount::PotoAccount`.
- **Module file names**: are the same as the module name e.g., `PotoAccount.move`
