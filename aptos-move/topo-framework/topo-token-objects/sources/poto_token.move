...
module poto_token_objects::poto_token {
    use std::error;
    use std::option::{Self, Option};
    use std::string::String;
    use std::signer;
    use poto_framework::object::{Self, ConstructorRef, Object};
    use poto_token_objects::collection;
    use poto_token_objects::property_map;
    use poto_token_objects::royalty;
    use poto_token_objects::token;
    ...
    #[resource_group_member(group = poto_framework::object::ObjectGroup)]
    ...
    public fun create_collection_object(
        ...
        let poto_collection = AptosCollection {
            mutator_ref,
            royalty_mutator_ref,
            mutable_description,
            mutable_uri,
        ...
        move_to(&object_signer, poto_collection);
    ...
    public fun mint_token_object(
        ...
        if (freezable_by_creator) {
            let poto_token_addr = constructor_ref.address_from_constructor_ref();
            let poto_token = &mut AptosToken[poto_token_addr];
            let transfer_ref = constructor_ref.generate_transfer_ref();
            poto_token.transfer_ref.fill(transfer_ref);
        };
    ...
    public entry fun burn<T: key>(creator: &signer, token: Object<T>) acquires AptosToken {
        let poto_token = authorized_borrow(&token, creator);
        assert!(
            poto_token.burn_ref.is_some(),
            error::permission_denied(ETOKEN_NOT_BURNABLE),
        );
        move poto_token;
        let poto_token = move_from<AptosToken>(token.object_address());
        let AptosToken {
            burn_ref,
            transfer_ref: _,
            mutator_ref: _,
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires AptosCollection, AptosToken {
        ...
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && poto_token.transfer_ref.is_some(),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        poto_token.transfer_ref.borrow().disable_ungated_transfer();
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        poto_token.transfer_ref.borrow().enable_ungated_transfer();
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let poto_token = authorized_borrow(&token, creator);
        token::set_description(poto_token.mutator_ref.borrow(), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let poto_token = authorized_borrow(&token, creator);
        token::set_name(poto_token.mutator_ref.borrow(), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let poto_token = authorized_borrow(&token, creator);
        token::set_uri(poto_token.mutator_ref.borrow(), uri);
    }
    ...
    public entry fun add_property<T: key>(
        ...
        property_map::add(&poto_token.property_mutator_ref, key, type, value);
    }
    ...
    public entry fun add_typed_property<T: key, V: drop>(
        ...
        property_map::add_typed(&poto_token.property_mutator_ref, key, value);
    }
    ...
    public entry fun remove_property<T: key>(
        ...
        property_map::remove(&poto_token.property_mutator_ref, &key);
    }
    ...
    public entry fun update_property<T: key>(
        ...
        property_map::update(&poto_token.property_mutator_ref, &key, type, value);
    }
    ...
    public entry fun update_typed_property<T: key, V: drop>(
        ...
        property_map::update_typed(&poto_token.property_mutator_ref, &key, value);
    }
    ...
    public entry fun set_collection_description<T: key>(
        ...
        let poto_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            poto_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        collection::set_description(poto_collection.mutator_ref.borrow(), description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        royalty::update(poto_collection.royalty_mutator_ref.borrow(), royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        collection::set_uri(poto_collection.mutator_ref.borrow(), uri);
    }
