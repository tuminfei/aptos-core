module poto_framework::genesis {
    use std::error;
    use std::fixed_point32;
    use std::vector;

    use poto_std::simple_map;

    use poto_framework::account;
    use poto_framework::aggregator_factory;
    use poto_framework::poto_account;
    use poto_framework::poto_coin::{Self, TopoCoin};
    use poto_framework::poto_governance;
    use poto_framework::block;
    use poto_framework::chain_id;
    ...
    use poto_framework::create_signer::create_signer;
    use poto_framework::gas_schedule;
    use poto_framework::nonce_validation;
    use poto_framework::reconfiguration;
    use poto_framework::stake;
    use poto_framework::staking_contract;
    use poto_framework::staking_config;
    use poto_framework::state_storage;
    use poto_framework::storage_gas;
    use poto_framework::timestamp;
    use poto_framework::transaction_fee;
    use poto_framework::transaction_validation;
    use poto_framework::version;
    use poto_framework::vesting;
    ...
    fun initialize(
        ...
        // to use this account.
        let (poto_framework_account, poto_framework_signer_cap) = account::create_framework_reserved_account(@poto_framework);
        // Initialize account configs on aptos framework account.
        account::initialize(&poto_framework_account);
        ...
        poto_governance::store_signer_cap(&poto_framework_account, @poto_framework, poto_framework_signer_cap);
        ...
        while (!framework_reserved_addresses.is_empty()) {
            ...
            poto_governance::store_signer_cap(&poto_framework_account, address, framework_signer_cap);
        };

        consensus_config::initialize(&poto_framework_account, consensus_config);
        execution_config::set(&poto_framework_account, execution_config);
        version::initialize(&poto_framework_account, initial_version);
        stake::initialize(&poto_framework_account);
        stake::initialize_pending_transaction_fee(&poto_framework_account);
        timestamp::set_time_has_started(&poto_framework_account);
        ...
        storage_gas::initialize(&poto_framework_account);
        gas_schedule::initialize(&poto_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&poto_framework_account);

        chain_id::initialize(&poto_framework_account, chain_id);
        reconfiguration::initialize(&poto_framework_account);
        block::initialize(&poto_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&poto_framework_account);
        nonce_validation::initialize(&poto_framework_account);
    }

    /// Genesis step 2: Initialize Aptos coin.
    fun initialize_poto_coin(poto_framework: &signer) {
        let (burn_cap, mint_cap) = poto_coin::initialize(poto_framework);

        coin::create_coin_conversion_map(poto_framework);
        coin::create_pairing<TopoCoin>(poto_framework);

        // Give stake module MintCapability<TopoCoin> so it can mint rewards.
        stake::store_poto_coin_mint_cap(poto_framework, mint_cap);
        // Give transaction_fee module BurnCapability<TopoCoin> so it can burn gas.
        transaction_fee::store_poto_coin_burn_cap(poto_framework, burn_cap);
        // Give transaction_fee module MintCapability<TopoCoin> so it can mint refunds.
        transaction_fee::store_poto_coin_mint_cap(poto_framework, mint_cap);
    }

    /// Only called for testnets and e2e tests.
    fun initialize_core_resources_and_poto_coin(
        ...
        poto_account::register_topo(&core_resources); // registers TOPO store
        poto_coin::configure_accounts_for_test(poto_framework, &core_resources, mint_cap);
    }

    fun create_accounts(poto_framework: &signer, accounts: vector<AccountMap>) {
    ...
    fun create_account(poto_framework: &signer, account_address: address, balance: u64): signer {
        ...
        if (coin::balance<TopoCoin>(account_address) == 0) {
            ...
            poto_coin::mint(poto_framework, account_address, balance);
        };
        account
    }
    ...
    fun create_initialize_validators_with_commission(
        ...
        validators.for_each_ref(|validator| {
            ...
            create_initialize_validator(poto_framework, validator, use_staking_contract);
        });

        // Destroy the aptos framework account's ability to mint coins now that we're done with setting up the initial
        // validators.
        poto_coin::destroy_mint_cap(poto_framework);
    ...
    fun create_initialize_validators(poto_framework: &signer, validators: vector<ValidatorConfiguration>) {
        ...
        create_initialize_validators_with_commission(poto_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(poto_framework, validator.owner_address, validator.stake_amount);
        create_account(poto_framework, validator.operator_address, 0);
        create_account(poto_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(poto_framework: &signer) {
        chain_status::set_genesis_end(poto_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        features::change_feature_flags_for_verification(poto_framework, vector[1, 2], vector[]);
        initialize_poto_coin(poto_framework);
        poto_governance::initialize_for_verification(
            poto_framework,
            min_voting_threshold,
            required_proposer_stake,
            voting_duration_secs
        );
        create_accounts(poto_framework, accounts);
        create_employee_validators(employee_vesting_start, employee_vesting_period_duration, employees);
        create_initialize_validators_with_commission(poto_framework, true, validators);
        set_genesis_end(poto_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@poto_framework), 1);
    ...
    fun test_create_account(poto_framework: &signer) {
        setup();
        initialize_poto_coin(poto_framework);

        let addr = @0x121341; // 01 -> 0a are taken
        let test_signer_before = create_account(poto_framework, addr, 15);
        let test_signer_after = create_account(poto_framework, addr, 500);
        assert!(test_signer_before == test_signer_after, 0);
        assert!(coin::balance<TopoCoin>(addr) == 15, 1);
    }

    #[test(poto_framework = @0x1)]
    fun test_create_accounts(poto_framework: &signer) {
        setup();
        initialize_poto_coin(poto_framework);
        ...
        create_account(poto_framework, addr0, 23456);
        assert!(coin::balance<TopoCoin>(addr0) == 12345, 2);
    }

    #[test(poto_framework = @0x1, root = @0xabcd)]
    fun test_create_root_account(poto_framework: &signer) {
        use poto_framework::aggregator_factory;
        use poto_framework::object;
        use poto_framework::primary_fungible_store;
        use poto_framework::fungible_asset::Metadata;
        use std::features;

        let feature = features::get_new_accounts_default_to_fa_topo_store_feature();
        features::change_feature_flags_for_testing(poto_framework, vector[feature], vector[]);

        aggregator_factory::initialize_aggregator_factory_for_test(poto_framework);
        ...
        let apt_metadata = object::address_to_object<Metadata>(@poto_fungible_asset);
