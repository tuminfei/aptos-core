...
module poto_framework::jwks {
    ...
    use std::vector;
    use poto_std::comparator::compare_u8_vector;
    use poto_std::copyable_any;
    use poto_std::copyable_any::Any;
    use poto_framework::chain_status;
    use poto_framework::config_buffer;
    use poto_framework::event::emit;
    ...
    const EINSTALL_FEDERATED_JWKS_AT_TOPOOS_FRAMEWORK: u64 = 7;
    ...
    public fun patch_federated_jwks(jwk_owner: &signer, patches: vector<Patch>) acquires FederatedJWKs {
        ...
        assert!(!system_addresses::is_aptos_framework_address(signer::address_of(jwk_owner)),
            error::invalid_argument(EINSTALL_FEDERATED_JWKS_AT_TOPOOS_FRAMEWORK)
        );
    ...
    public fun try_get_patched_jwk(issuer: vector<u8>, jwk_id: vector<u8>): Option<JWK> acquires PatchedJWKs {
        let jwks = &borrow_global<PatchedJWKs>(@poto_framework).jwks;
    ...
    public fun upsert_oidc_provider(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = borrow_global_mut<SupportedOIDCProviders>(@poto_framework);
    ...
    public fun upsert_oidc_provider_for_next_epoch(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            *borrow_global<SupportedOIDCProviders>(@poto_framework)
        };
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires SupportedOIDCProviders {
        ...
        if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            if (exists<SupportedOIDCProviders>(@poto_framework)) {
                *borrow_global_mut<SupportedOIDCProviders>(@poto_framework) = new_config;
            } else {
    ...
    public fun set_patches(fx: &signer, patches: vector<Patch>) acquires Patches, PatchedJWKs, ObservedJWKs {
        system_addresses::assert_aptos_framework(fx);
        borrow_global_mut<Patches>(@poto_framework).patches = patches;
    ...
    public fun upsert_into_observed_jwks(fx: &signer, provider_jwks_vec: vector<ProviderJWKs>) acquires ObservedJWKs, PatchedJWKs, Patches {
        system_addresses::assert_aptos_framework(fx);
        let observed_jwks = borrow_global_mut<ObservedJWKs>(@poto_framework);
    ...
    fun regenerate_patched_jwks() acquires PatchedJWKs, Patches, ObservedJWKs {
        let jwks = borrow_global<ObservedJWKs>(@poto_framework).jwks;
        let patches = borrow_global<Patches>(@poto_framework);
        patches.patches.for_each_ref(|obj|{
            let patch: &Patch = obj;
            apply_patch(&mut jwks, *patch);
        });
        *borrow_global_mut<PatchedJWKs>(@poto_framework) = PatchedJWKs { jwks };
    }
    ...
    #[test_only]
    fun initialize_for_test(poto_framework: &signer) {
        create_account_for_test(@poto_framework);
        reconfiguration::initialize_for_test(poto_framework);
        initialize(poto_framework);
    }

    #[test(fx = @poto_framework)]
    fun test_observed_jwks_operations(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@poto_framework).jwks, 1);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@poto_framework).jwks, 2);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@poto_framework).jwks, 3);

        remove_issuer_from_observed_jwks(fx, b"alice");
        let expected = AllProvidersJWKs { entries: vector[bob_jwks_v1] };
        assert!(expected == borrow_global<ObservedJWKs>(@poto_framework).jwks, 4);
    }
    ...
    fun test_observed_jwks_operations_per_key_mode(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<PatchedJWKs>(@poto_framework).jwks, 999);
    ...
    fun test_patched_jwks(poto_framework: signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        upsert_into_observed_jwks(&poto_framework, vector [
            ProviderJWKs {
                issuer: b"alice",
                version: 111,
                jwks: vector[jwk_0, jwk_1],
        ...
        set_patches(&poto_framework, vector[
