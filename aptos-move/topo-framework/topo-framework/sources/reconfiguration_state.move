...
module poto_framework::reconfiguration_state {
    use std::error;
    ...
    public(friend) fun is_in_progress(): bool acquires State {
        ...
        let state = borrow_global<State>(@poto_framework);
    ...
    public(friend) fun on_reconfig_start() acquires State {
        if (exists<State>(@poto_framework)) {
            let state = borrow_global_mut<State>(@poto_framework);
            let variant_type_name = *state.variant.type_name().bytes();
