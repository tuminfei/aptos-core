...
module poto_framework::fungible_asset {
    ...
    use poto_framework::object::{Self, Object, ConstructorRef, DeleteRef, ExtendRef};
    ...
    friend poto_framework::primary_fungible_store;
    friend poto_framework::poto_account;

    friend poto_framework::dispatchable_fungible_asset;
    ...
    const ETOPO_NOT_DISPATCHABLE: u64 = 31;
    /// Flag for Concurrent Supply not enabled
    ...
    inline fun register_dispatch_function_sanity_check(
        ...
        assert!(
            constructor_ref.address_from_constructor_ref()
                != @poto_fungible_asset,
            error::permission_denied(ETOPO_NOT_DISPATCHABLE)
        );
        assert!(
            !constructor_ref.can_generate_delete_ref(),
            error::invalid_argument(EOBJECT_IS_DELETABLE)
    ...
    fun has_deposit_dispatch_function(
        ...
        if (metadata_addr != @poto_fungible_asset
            && exists<DispatchFunctionStore>(metadata_addr)) {
    ...
    #[test(fx = @poto_framework, creator = @0xcafe)]
