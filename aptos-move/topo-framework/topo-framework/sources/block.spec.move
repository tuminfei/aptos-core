spec poto_framework::block {
    ...
    /// Implementation: The initialize and  update_epoch_interval_microsecs functions ensure that only poto_framework
    /// can call them.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@poto_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@poto_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@poto_framework).new_block_events) == 0;
    ...
    spec initialize(poto_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@poto_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@poto_framework);
        aborts_if !exists<reconfiguration::Configuration>(@poto_framework);
