spec poto_framework::poto_coin {
    ...
    spec initialize(poto_framework: &signer): (BurnCapability<TopoCoin>, MintCapability<TopoCoin>) {
        use poto_framework::aggregator_factory;
        use poto_framework::permissioned_signer;

        pragma verify = false;

        aborts_if permissioned_signer::spec_is_permissioned_signer(poto_framework);
        ...
        /// [high-level-req-3]
        ensures global<MintCapStore>(addr).mint_cap ==  MintCapability<TopoCoin> {};
        ensures exists<coin::CoinInfo<TopoCoin>>(addr);
        ensures result_1 == BurnCapability<TopoCoin> {};
        ensures result_2 == MintCapability<TopoCoin> {};
    }

    spec destroy_mint_cap {
        let addr = signer::address_of(poto_framework);
        aborts_if addr != @poto_framework;
        aborts_if !exists<MintCapStore>(@poto_framework);
    }
