spec poto_framework::vesting {
    ...
    /// period.
    /// Enforcement: Audited the check for the end of vesting cliff: [https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/topo-framework/sources/vesting.move#L566](vest) module.
    ///
    /// No.: 5
    ...
    spec create_vesting_contract {
        ...
        pragma verify = false;
        /// [high-level-req-10]
        aborts_if withdrawal_address == @poto_framework || withdrawal_address == @vm_reserved;
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<TopoCoin>>(resource_addr) && !poto_std::type_info::spec_is_struct<TopoCoin>();
    ...
    spec schema DistributeInternalAbortsIf {
        ...
        aborts_if !exists<stake::ValidatorSet>(@poto_framework);
        let validator_set = global<stake::ValidatorSet>(@poto_framework);
