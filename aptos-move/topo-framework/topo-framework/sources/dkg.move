...
module poto_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use poto_framework::event::emit;
    use poto_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@poto_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@poto_framework)) {
            borrow_global<DKGState>(@poto_framework).in_progress
        } else {
            option::none()
        }
    }
