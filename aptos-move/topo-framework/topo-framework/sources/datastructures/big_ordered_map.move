...
module poto_framework::big_ordered_map {
    use std::error;
    use std::vector;
    use std::option::{Self as option, Option};
    use std::bcs;
    use poto_std::cmp;
    use poto_std::storage_slots_allocator::{Self, StorageSlotsAllocator, StoredSlot};
    use poto_std::math64::{max, min};
    use poto_framework::ordered_map::{Self, OrderedMap};

    #[test_only]
    friend poto_framework::big_ordered_map_test;
    ...
    public fun new_with_type_size_hints<K: store, V: store>(avg_key_bytes: u64, max_key_bytes: u64, avg_value_bytes: u64, max_value_bytes: u64): BigOrderedMap<K, V> {
        ...
        assert!(leaf_max_degree_from_max >= (LEAF_MIN_DEGREE as u64), error::invalid_argument(EINVALID_CONFIG_PARAMETER));
    ...
    public(friend) fun print_map<K: store, V: store>(self: &BigOrderedMap<K, V>) {
        // uncomment to debug:
        // poto_std::debug::print(&std::string::utf8(b"print map"));
        // poto_std::debug::print(self);
    ...
    fun print_map_for_node<K: store + copy + drop, V: store>(self: &BigOrderedMap<K, V>, node_index: u64, level: u64) {
        ...
        poto_std::debug::print(&level);
        poto_std::debug::print(&node_index);
        poto_std::debug::print(node);
    ...
    fun validate_subtree<K: drop + copy + store, V: store>(self: &BigOrderedMap<K, V>, node_index: u64, expected_lower_bound_key: Option<K>, expected_max_key: Option<K>) {
        ...
        if (node_index != ROOT_INDEX) {
            ...
            assert!(len * 2 >= self.get_max_degree(node.is_leaf), error::invalid_state(EINTERNAL_INVARIANT_BROKEN));
        };
