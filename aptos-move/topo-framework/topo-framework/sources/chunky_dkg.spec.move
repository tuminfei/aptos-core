spec poto_framework::chunky_dkg {
    ...
    spec start(
        ...
        aborts_if !exists<ChunkyDKGState>(@poto_framework);
        aborts_if !exists<timestamp::CurrentTimeMicroseconds>(@poto_framework);
    }

    spec finish(aggregated_subtranscript: vector<u8>) {
        use std::option;
        requires exists<ChunkyDKGState>(@poto_framework);
        requires option::is_some(global<ChunkyDKGState>(@poto_framework).in_progress);
        aborts_if false;
    }
