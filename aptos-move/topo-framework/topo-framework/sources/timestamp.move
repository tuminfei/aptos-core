...
module poto_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(poto_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@poto_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@poto_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@poto_framework).microseconds
    }
