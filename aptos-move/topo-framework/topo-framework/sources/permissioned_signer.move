...
/// for blind signing.
module poto_framework::permissioned_signer {
    use std::features;
    use std::signer;
    use std::error;
    use std::vector;
    use std::option::{Option, Self};
    use poto_std::copyable_any::{Self, Any};
    use poto_framework::big_ordered_map::{Self, BigOrderedMap};
    use poto_framework::create_signer::create_signer;
    use poto_framework::transaction_context::generate_auid_address;
    ...
    fun signer_address_roundtrip(
        ...
        let poto_framework = create_signer(@0x1);
    ...
    #[test(creator = @0xcafe)]
    #[expected_failure(abort_code = 0x1C5, location = poto_std::bcs)]
