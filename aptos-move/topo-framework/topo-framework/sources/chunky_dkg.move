...
module poto_framework::chunky_dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use poto_framework::event::emit;
    use poto_framework::chunky_dkg_config::ChunkyDKGConfig;
    ...
    public fun initialize(poto_framework: &signer) {
        ...
        if (!exists<ChunkyDKGState>(@poto_framework)) {
    ...
    public(friend) fun start(
        ...
        let chunky_dkg_state = borrow_global_mut<ChunkyDKGState>(@poto_framework);
    ...
    public fun incomplete_session(): Option<ChunkyDKGSessionState> acquires ChunkyDKGState {
        if (exists<ChunkyDKGState>(@poto_framework)) {
            borrow_global<ChunkyDKGState>(@poto_framework).in_progress
        } else {
            option::none()
        }
    }
