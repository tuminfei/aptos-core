...
module poto_framework::storage_gas {
    ...
    public(friend) fun set_config(poto_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_aptos_framework(poto_framework);
        *borrow_global_mut<StorageGasConfig>(@poto_framework) = config;
    }
    ...
    public fun initialize(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        assert!(
            !exists<StorageGasConfig>(@poto_framework),
        ...
        move_to(poto_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@poto_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(poto_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@poto_framework);
        let gas = borrow_global_mut<StorageGas>(@poto_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@poto_framework);
