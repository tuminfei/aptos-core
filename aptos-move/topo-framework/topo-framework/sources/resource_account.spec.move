spec poto_framework::resource_account {
    /// <high-level-req>
    /// No.: 1
    ...
    spec create_resource_account_and_fund(
        ...
        include poto_account::WithdrawAbortsIf<TopoCoin>{from: origin, amount: fund_amount};
        include poto_account::GuidAbortsIf<TopoCoin>{to: resource_addr};
        include RotateAccountAuthenticationKeyAndStoreCapabilityAbortsIfWithoutAccountLimit;

        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<TopoCoin>(resource_addr) && coin_store_resource.frozen;
        /// [high-level-req-3]
        ensures exists<poto_framework::coin::CoinStore<TopoCoin>>(resource_addr);
    }
    ...
    spec rotate_account_authentication_key_and_store_capability(
        ...
        ensures vector::length(optional_auth_key) != 0 ==>
            global<poto_framework::account::Account>(resource_addr).authentication_key == optional_auth_key;
    }
