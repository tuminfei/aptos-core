spec poto_framework::reconfiguration {
    /// <high-level-req>
    /// No.: 1
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==>
            exists<Configuration>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==>
    ...
    spec initialize(poto_framework: &signer) {
        ...
        let addr = signer::address_of(poto_framework);
        let post config = global<Configuration>(@poto_framework);
        requires exists<Account>(addr);
        aborts_if !(global<Account>(addr).guid_creation_num == 2);
        aborts_if exists<Configuration>(@poto_framework);
    ...
    spec current_epoch(): u64 {
        aborts_if !exists<Configuration>(@poto_framework);
        ensures result == global<Configuration>(@poto_framework).epoch;
    }

    spec disable_reconfiguration(poto_framework: &signer) {
        include AbortsIfNotAptosFramework;
        aborts_if exists<DisableReconfiguration>(@poto_framework);
        ensures exists<DisableReconfiguration>(@poto_framework);
    }

    /// Make sure the caller is admin and check the resource DisableReconfiguration.
    spec enable_reconfiguration(poto_framework: &signer) {
        use poto_framework::reconfiguration::{DisableReconfiguration};
        include AbortsIfNotAptosFramework;
        aborts_if !exists<DisableReconfiguration>(@poto_framework);
        ensures !exists<DisableReconfiguration>(@poto_framework);
    }
    ...
    spec emit_genesis_reconfiguration_event {
        use poto_framework::reconfiguration::{Configuration};

        aborts_if !exists<Configuration>(@poto_framework);
        let config_ref = global<Configuration>(@poto_framework);
        aborts_if !(config_ref.epoch == 0 && config_ref.last_reconfiguration_time == 0);
        ensures global<Configuration>(@poto_framework).epoch == 1;
    }

    spec last_reconfiguration_time {
        aborts_if !exists<Configuration>(@poto_framework);
        ensures result
            == global<Configuration>(@poto_framework).last_reconfiguration_time;
    }
    ...
    spec reconfigure {
        ...
        include success ==> poto_coin::ExistsTopoCoin;
        aborts_if false;
        ...
        ensures success ==>
            global<Configuration>(@poto_framework).epoch
                == old(global<Configuration>(@poto_framework).epoch) + 1;
        ensures success ==>
            global<Configuration>(@poto_framework).last_reconfiguration_time
                == timestamp::spec_now_microseconds();
        ...
        ensures !success ==>
            global<Configuration>(@poto_framework).epoch
                == old(global<Configuration>(@poto_framework).epoch);
    }
    ...
    spec reconfiguration_enabled {
        ...
        /// [high-level-req-2]
        aborts_if false;
        ensures result == !exists<DisableReconfiguration>(@poto_framework);
    }
}
