...
module poto_framework::aggregator_factory {
    use std::error;
    ...
    friend poto_framework::optional_aggregator;
    ...
    public(friend) fun create_aggregator_internal(): Aggregator acquires AggregatorFactory {
        assert!(
            exists<AggregatorFactory>(@poto_framework),
            error::not_found(EAGGREGATOR_FACTORY_NOT_FOUND)
        );

        let aggregator_factory = borrow_global_mut<AggregatorFactory>(@poto_framework);
    ...
    #[test_only]
    public fun initialize_aggregator_factory_for_test(poto_framework: &signer) {
        initialize_aggregator_factory(poto_framework);
    }
