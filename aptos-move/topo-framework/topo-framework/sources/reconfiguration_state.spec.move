spec poto_framework::reconfiguration_state {

    spec module {
        use poto_framework::chain_status;
        invariant [suspendable] chain_status::is_operating() ==> exists<State>(@poto_framework);
    }

    spec initialize(fx: &signer) {
        use std::signer;
        use poto_std::from_bcs;
        aborts_if signer::address_of(fx) != @poto_framework;
        let post post_state = global<State>(@poto_framework);
        ensures exists<State>(@poto_framework);
        ensures !exists<State>(@poto_framework) ==> from_bcs::deserializable<StateInactive>(post_state.variant.data);
    }
    ...
    spec fun spec_is_in_progress(): bool {
        if (!exists<State>(@poto_framework)) {
            false
        } else {
            copyable_any::type_name(global<State>(@poto_framework).variant).bytes == b"0x1::reconfiguration_state::StateActive"
        }
    }
    ...
    spec on_reconfig_start {
        ...
        let pre_state = global<State>(@poto_framework);
        let post post_state = global<State>(@poto_framework);
        ensures (exists<State>(@poto_framework) && copyable_any::type_name(pre_state.variant).bytes
    ...
    spec fun spec_start_time_secs(): u64 {
        use poto_std::from_bcs;
        let state = global<State>(@poto_framework);
        from_bcs::deserialize<StateActive>(state.variant.data).start_time_secs
    }

    spec schema StartTimeSecsRequirement {
        requires exists<State>(@poto_framework);
        requires copyable_any::type_name(global<State>(@poto_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive";
        include UnpackRequiresStateActive {
            x:  global<State>(@poto_framework).variant
        };
    }
    ...
    spec schema StartTimeSecsAbortsIf {
        aborts_if !exists<State>(@poto_framework);
        include  copyable_any::type_name(global<State>(@poto_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive" ==>
        copyable_any::UnpackAbortsIf<StateActive> {
            self: global<State>(@poto_framework).variant
        };
        aborts_if copyable_any::type_name(global<State>(@poto_framework).variant).bytes
            != b"0x1::reconfiguration_state::StateActive";
    }
