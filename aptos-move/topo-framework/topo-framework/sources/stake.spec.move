spec poto_framework::stake {
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@poto_framework) ==>
        ...
        invariant [suspendable] chain_status::is_operating() ==>
            exists<TopoCoinCapabilities>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==>
            exists<ValidatorPerformance>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==>
            exists<ValidatorSet>(@poto_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (
            reconfiguration_state::spec_is_in_progress()
                && old(exists<ValidatorSet>(@poto_framework))
        ) ==>
            old(global<ValidatorSet>(@poto_framework))
                == global<ValidatorSet>(@poto_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@poto_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@poto_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@poto_framework)
    ...
    spec initialize(poto_framework: &signer) {
        ...
        let poto_addr = signer::address_of(poto_framework);
        aborts_if !system_addresses::is_aptos_framework_address(poto_addr);
        aborts_if exists<ValidatorSet>(poto_addr);
        aborts_if exists<ValidatorPerformance>(poto_addr);
        ensures exists<ValidatorSet>(poto_addr);
        ensures global<ValidatorSet>(poto_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(poto_addr);
    }
    ...
    spec join_validator_set(operator: &signer, pool_address: address) {
        ...
        aborts_if !exists<StakingConfig>(@poto_framework);
        aborts_if !exists<ValidatorSet>(@poto_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@poto_framework);
        let post p_validator_set = global<ValidatorSet>(@poto_framework);
    ...
    spec withdraw(owner: &signer, withdraw_amount: u64) {
        ...
        aborts_if bool_find_validator
            && !exists<timestamp::CurrentTimeMicroseconds>(@poto_framework);
    ...
    spec leave_validator_set(operator: &signer, pool_address: address) {
        ...
        let post post_validator_set = global<ValidatorSet>(@poto_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = poto_std::math64::min(amount, pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@poto_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = poto_std::math64::min(
    ...
    spec on_new_epoch {
        ...
        include poto_framework::poto_coin::ExistsTopoCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@poto_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@poto_framework);
    ...
    spec schema UpdateStakePoolAbortsIf {
        ...
        let poto_addr = type_info::type_of<TopoCoin>().account_address;
    ...
    spec schema GetReconfigStartTimeRequirement {
        requires exists<timestamp::CurrentTimeMicroseconds>(@poto_framework);
        include reconfiguration_state::StartTimeSecsRequirement;
    }

    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@poto_framework)) {
    ...
    spec configure_allowed_validators(poto_framework: &signer, accounts: vector<address>) {
        let poto_framework_address = signer::address_of(poto_framework);
        aborts_if !system_addresses::is_aptos_framework_address(poto_framework_address);
        let post allowed = global<AllowedValidators>(poto_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@poto_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<TopoCoinCapabilities>(@poto_framework);
        requires exists<ValidatorPerformance>(@poto_framework);
        requires exists<ValidatorSet>(@poto_framework);
        requires exists<StakingConfig>(@poto_framework);
        requires exists<StakingRewardsConfig>(@poto_framework)
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate =
                global<staking_config::StakingRewardsConfig>(@poto_framework).rewards_rate;
