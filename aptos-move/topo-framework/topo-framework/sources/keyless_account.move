...
module poto_framework::keyless_account {
    use std::bn254_algebra;
    use std::config_buffer;
    use std::option::Option;
    use std::signer;
    use std::string::String;
    use poto_std::crypto_algebra;
    use poto_std::ed25519;
    use poto_framework::chain_status;
    use poto_framework::system_addresses;

    // The `poto_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = poto_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@poto_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@poto_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@poto_framework)) {
                *borrow_global_mut<Configuration>(@poto_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
