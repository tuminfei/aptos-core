module poto_framework::nonce_validation {
    use poto_std::table::{Self, Table};
    use poto_std::timestamp;
    use poto_std::big_ordered_map::{Self, BigOrderedMap};
    use poto_std::poto_hash::sip_hash_from_value;
    use poto_std::error;
    ...
    public(friend) fun initialize(poto_framework: &signer) {
        initialize_nonce_table(poto_framework);
    }

    public entry fun initialize_nonce_table(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        if (!exists<NonceHistory>(@poto_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(poto_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@poto_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@poto_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@poto_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
