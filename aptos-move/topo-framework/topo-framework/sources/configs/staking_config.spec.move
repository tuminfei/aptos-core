spec poto_framework::staking_config {
    /// <high-level-req>
    /// No.: 1
    ...
    /// update_voting_power_increase_limit functions, which ensure that the signer is poto_framework using the
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StakingConfig>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StakingRewardsConfig>(@poto_framework);
        pragma verify = true;
    ...
    spec get_reward_rate(config: &StakingConfig): (u64, u64) {
        ...
        ensures (features::spec_periodical_reward_rate_decrease_enabled() &&
            (global<StakingRewardsConfig>(@poto_framework).rewards_rate.value as u64) != 0) ==>
                result_2 <= MAX_U64;
    }

    spec reward_rate(): (u64, u64) {
        let config = global<StakingConfig>(@poto_framework);
        aborts_if !exists<StakingConfig>(@poto_framework);
        include StakingRewardsConfigRequirement;
        ensures (features::spec_periodical_reward_rate_decrease_enabled() &&
            (global<StakingRewardsConfig>(@poto_framework).rewards_rate.value as u64) != 0) ==>
    ...
    spec calculate_and_save_latest_epoch_rewards_rate(): FixedPoint64 {
        pragma verify_duration_estimate = 120;
        aborts_if !exists<StakingRewardsConfig>(@poto_framework);
    ...
    spec update_required_stake(
        ...
        aborts_if minimum_stake > maximum_stake || maximum_stake == 0;
        aborts_if !exists<StakingConfig>(@poto_framework);
        ensures global<StakingConfig>(@poto_framework).minimum_stake == minimum_stake &&
            global<StakingConfig>(@poto_framework).maximum_stake == maximum_stake;
    }
    ...
    spec update_recurring_lockup_duration_secs(
        ...
        aborts_if !exists<StakingConfig>(@poto_framework);
        ensures global<StakingConfig>(@poto_framework).recurring_lockup_duration_secs == new_recurring_lockup_duration_secs;
    }
    ...
    spec update_rewards_rate(
        ...
        let post staking_config = global<StakingConfig>(@poto_framework);
    ...
    spec update_rewards_config(
        ...
        /// [high-level-req-1.6]
        aborts_if addr != @poto_framework;
        aborts_if global<StakingRewardsConfig>(@poto_framework).rewards_rate_period_in_secs != rewards_rate_period_in_secs;
        include StakingRewardsConfigValidationAbortsIf;
        aborts_if !exists<StakingRewardsConfig>(addr);
        let post staking_rewards_config = global<StakingRewardsConfig>(@poto_framework);
    ...
    spec update_voting_power_increase_limit(
        ...
        aborts_if !exists<StakingConfig>(@poto_framework);
        ensures global<StakingConfig>(@poto_framework).voting_power_increase_limit == new_voting_power_increase_limit;
    }
    ...
    spec schema StakingRewardsConfigEnabledRequirement {
        ...
        let staking_rewards_config = global<StakingRewardsConfig>(@poto_framework);
