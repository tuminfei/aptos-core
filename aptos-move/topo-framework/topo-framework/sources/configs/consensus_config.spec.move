spec poto_framework::consensus_config {
    /// <high-level-req>
    /// No.: 1
    ...
    /// that the signer is the poto_framework and then assigns the ConsensusConfig resource to it.
    /// Enforcement: Formally verified via [high-level-req-1](initialize).
    ///
    /// No.: 2
    /// Requirement: Only aptos framework account is allowed to update the consensus configuration.
    /// Criticality: Medium
    /// Implementation: The consensus_config::set function ensures that the signer is poto_framework.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<ConsensusConfig>(@poto_framework);
    }

    /// Ensure caller is admin.
    /// Aborts if StateStorageUsage already exists.
    spec initialize(poto_framework: &signer, config: vector<u8>) {
        use std::signer;
        let addr = signer::address_of(poto_framework);
        /// [high-level-req-1]
        aborts_if !system_addresses::is_aptos_framework_address(addr);
        aborts_if exists<ConsensusConfig>(@poto_framework);
        /// [high-level-req-3.1]
    ...
    spec set(account: &signer, config: vector<u8>) {
        ...
        ensures global<ConsensusConfig>(@poto_framework).config == config;
    }
    ...
    spec validator_txn_enabled(): bool {
        pragma opaque;
        aborts_if !exists<ConsensusConfig>(@poto_framework);
        ensures [abstract] result == spec_validator_txn_enabled_internal(global<ConsensusConfig>(@poto_framework).config);
    }
