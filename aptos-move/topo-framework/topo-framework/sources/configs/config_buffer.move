...
module poto_framework::config_buffer {
    use std::error;
    use std::string::String;
    use poto_std::any;
    use poto_std::any::Any;
    use poto_std::simple_map;
    use poto_std::simple_map::SimpleMap;
    use poto_std::type_info;
    use poto_framework::system_addresses;

    friend poto_framework::chunky_dkg_config;
    friend poto_framework::consensus_config;
    friend poto_framework::decryption;
    friend poto_framework::execution_config;
    friend poto_framework::gas_schedule;
    friend poto_framework::jwks;
    friend poto_framework::jwk_consensus_config;
    friend poto_framework::keyless_account;
    friend poto_framework::randomness_api_v0_config;
    friend poto_framework::randomness_config;
    friend poto_framework::randomness_config_seqnum;
    ...
    public fun initialize(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        if (!exists<PendingConfigs>(@poto_framework)) {
            move_to(poto_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@poto_framework)) {
            let config = borrow_global<PendingConfigs>(@poto_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@poto_framework);
