...
module poto_framework::version {
    ...
    public(friend) fun initialize(poto_framework: &signer, initial_version: u64) {
        system_addresses::assert_aptos_framework(poto_framework);

        move_to(poto_framework, Version { major: initial_version });
        ...
        move_to(poto_framework, SetVersionCapability {});
    }
    ...
    public entry fun set_version(account: &signer, major: u64) acquires Version {
        ...
        let old_major = borrow_global<Version>(@poto_framework).major;
        assert!(old_major < major, error::invalid_argument(EINVALID_MAJOR_VERSION_NUMBER));

        let config = borrow_global_mut<Version>(@poto_framework);
        config.major = major;
    ...
    /// Example usage:
    /// - `poto_framework::version::set_for_next_epoch(&framework_signer, new_version);`
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires Version {
        ...
        if (config_buffer::does_exist<Version>()) {
            ...
            if (exists<Version>(@poto_framework)) {
                *borrow_global_mut<Version>(@poto_framework) = new_value;
            } else {
    ...
    public entry fun test_set_version(poto_framework: signer) acquires Version {
        initialize(&poto_framework, 1);
        assert!(borrow_global<Version>(@poto_framework).major == 1, 0);
        set_version(&poto_framework, 2);
        assert!(borrow_global<Version>(@poto_framework).major == 2, 1);
    }

    #[test(poto_framework = @poto_framework, core_resources = @core_resources)]
    ...
    #[test(poto_framework = @poto_framework, random_account = @0x123)]
