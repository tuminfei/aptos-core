...
module poto_framework::randomness_config {
    use poto_std::copyable_any;
    use poto_std::copyable_any::Any;
    use poto_std::fixed_point64::FixedPoint64;
    ...
    public fun initialize(framework: &signer, config: RandomnessConfig) {
        ...
        if (!exists<RandomnessConfig>(@poto_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfig {
        ...
        if (config_buffer::does_exist<RandomnessConfig>()) {
            ...
            if (exists<RandomnessConfig>(@poto_framework)) {
                *borrow_global_mut<RandomnessConfig>(@poto_framework) = new_config;
            } else {
    ...
    public fun enabled(): bool acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@poto_framework)) {
            let config = borrow_global<RandomnessConfig>(@poto_framework);
    ...
    public fun current(): RandomnessConfig acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@poto_framework)) {
            *borrow_global<RandomnessConfig>(@poto_framework)
        } else {
            new_off()
        }
    }
