spec poto_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `poto_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use poto_framework::timestamp::{CurrentTimeMicroseconds};
        use poto_framework::chain_id::{ChainId};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@poto_framework);
        aborts_if !(timestamp::now_seconds() < txn_expiration_time);

        aborts_if !exists<ChainId>(@poto_framework);
    ...
    /// `TopoCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `TopoCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use poto_framework::account::{Account};
        use poto_framework::poto_coin::{TopoCoin};
        use poto_framework::coin;
        use poto_framework::coin::{CoinStore, CoinInfo};
        use poto_framework::optional_aggregator;
        use poto_framework::transaction_fee::{TopoCoinCapabilities, TopoCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<TopoCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<TopoCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);

        aborts_if !exists<CoinStore<TopoCoin>>(gas_payer);
        ...
        let apt_addr = type_info::type_of<TopoCoin>().account_address;
        let maybe_topo_supply = global<CoinInfo<TopoCoin>>(apt_addr).supply;
        let total_supply_enabled = option::is_some(maybe_topo_supply);
        let apt_supply = option::borrow(maybe_topo_supply);
        let apt_supply_value = optional_aggregator::optional_aggregator_value(apt_supply);
        let post post_maybe_topo_supply = global<CoinInfo<TopoCoin>>(apt_addr).supply;
        ...
        aborts_if amount_to_burn > 0 && !exists<TopoCoinCapabilities>(@poto_framework);
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<TopoCoin>>(apt_addr);
        aborts_if amount_to_burn > 0 && total_supply_enabled && apt_supply_value < amount_to_burn;
        ensures total_supply_enabled ==> apt_supply_value - amount_to_burn == post_topo_supply_value;

        // Check minting.
        let amount_to_mint = storage_fee_refunded - transaction_fee_amount;
        let total_supply = coin::supply<TopoCoin>;
        let post post_total_supply = coin::supply<TopoCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<TopoCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<TopoCoinMintCapability>(@poto_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<TopoCoin>>(poto_addr);
        include coin::CoinAddAbortsIf<TopoCoin> { amount: amount_to_mint };
    }
}
