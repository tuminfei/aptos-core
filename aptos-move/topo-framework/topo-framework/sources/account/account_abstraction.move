module poto_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@poto_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    public entry fun initialize(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        move_to(
            poto_framework,
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @poto_fungible_asset)
    }
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@poto_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@poto_framework].auth_functions
   }
