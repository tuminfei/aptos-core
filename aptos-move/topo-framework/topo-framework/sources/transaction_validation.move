module poto_framework::transaction_validation {
    use std::error;
    use std::features;
    use std::option;
    ...
    use poto_framework::account_abstraction;
    ...
    public(friend) fun initialize(
        ...
        move_to(poto_framework, TransactionValidation {
    ...
    fun prologue_common(
        ...
        if (!skip_gas_payment(
            ...
            assert!(
                poto_account::is_fungible_balance_at_least(gas_payer_address, max_transaction_fee),
    ...
    fun epilogue_gas_payer_extended(
        ...
        if (!skip_gas_payment(is_simulation, gas_payer)) {
            assert!(
                poto_account::is_fungible_balance_at_least(gas_payer, transaction_fee_amount),
    ...
    fun unified_epilogue_v2(
        ...
        if (!skip_gas_payment(
            ...
            assert!(
                poto_account::is_fungible_balance_at_least(gas_payer_address, transaction_fee_amount),
                error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
            );
