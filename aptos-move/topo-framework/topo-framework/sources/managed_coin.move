...
module poto_framework::managed_coin {
    use std::string;
    use std::error;
    use std::signer;

    use poto_framework::coin::{Self, BurnCapability, FreezeCapability, MintCapability, destroy_burn_cap,
    ...
    #[test(framework = @poto_framework, source = @0xa11ce, destination = @0xb0b, mod_account = @0x1)]
    public entry fun test_end_to_end(
        ...
        poto_framework::account::create_account_for_test(source_addr);
        poto_framework::account::create_account_for_test(destination_addr);
        poto_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        poto_framework::coin::create_coin_conversion_map(&framework);
    ...
    public entry fun fail_mint(
        ...
        poto_framework::account::create_account_for_test(source_addr);
        poto_framework::account::create_account_for_test(signer::address_of(&destination));
        poto_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        poto_framework::coin::create_coin_conversion_map(&framework);
