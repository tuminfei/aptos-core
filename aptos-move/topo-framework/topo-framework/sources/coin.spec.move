spec poto_framework::coin {
    ...
    /// Can only be updated by `@poto_framework`.
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@poto_framework)) {
            let map =
                global<CoinConversionMap>(@poto_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@poto_framework`.
    ...
    // `account` must be `@poto_framework`.
    spec initialize_with_parallelizable_supply<CoinType>(
        ...
        aborts_if monitor_supply
            && !exists<aggregator_factory::AggregatorFactory>(@poto_framework);
