spec poto_framework::poto_account {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Aptos account, the account should become registered to receive TopoCoin.
    /// Criticality: Critical
    ...
    /// exist; then, register TopoCoin for that account if a particular is unregistered before transferring the amount.
    /// Criticality: Critical
    /// Implementation: The transfer function checks if the recipient account exists. If the account does not exist,
    /// the function creates one and registers the account to TopoCoin if not registered.
    ...
    ///
    /// No.: 7
    /// Requirement: When performing a batch transfer of Topo Coin and/or a batch transfer of a custom coin type, it
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @poto_framework || auth_key == @poto_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        include GuidAbortsIf<TopoCoin>;
        include WithdrawAbortsIf<TopoCoin>{from: source};
        include TransferEnsures<TopoCoin>;

        aborts_if exists<coin::CoinStore<TopoCoin>>(to) && global<coin::CoinStore<TopoCoin>>(to).frozen;
        /// [high-level-req-5]
        ensures exists<poto_framework::account::Account>(to);
        ensures exists<coin::CoinStore<TopoCoin>>(to);
    }
    ...
    spec assert_account_is_registered_for_topo(addr: address) {
        pragma aborts_if_is_partial;
        // aborts_if !account::spec_exists_at(addr);
        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<TopoCoin>(addr);
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<TopoCoin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @poto_framework || recipients[i] == @poto_token);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<TopoCoin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<TopoCoin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<TopoCoin>>(recipients[i]) && global<coin::CoinStore<TopoCoin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<TopoCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<TopoCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) {
        ...
        /// [high-level-spec-6]
        ensures exists<poto_framework::account::Account>(to);
        ensures exists<poto_framework::coin::CoinStore<CoinType>>(to);
    ...
    spec register_topo(account_signer: &signer) {
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @poto_framework || to == @poto_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<poto_framework::account::Account>(to);
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<TopoCoin>();
    }
