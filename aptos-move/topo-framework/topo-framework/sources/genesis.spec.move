spec poto_framework::genesis {
    ...
    /// Enforcement: Formally verified via [high-level-req-3](initialize_poto_coin).
    ///
    /// No.: 4
    ...
    spec initialize {
        ...
        /// [high-level-req-1]
        ensures exists<poto_governance::GovernanceResponsbility>(@poto_framework);
        ensures exists<consensus_config::ConsensusConfig>(@poto_framework);
        ensures exists<execution_config::ExecutionConfig>(@poto_framework);
        ensures exists<version::Version>(@poto_framework);
        ensures exists<stake::ValidatorSet>(@poto_framework);
        ensures exists<stake::ValidatorPerformance>(@poto_framework);
        ensures exists<storage_gas::StorageGasConfig>(@poto_framework);
        ensures exists<storage_gas::StorageGas>(@poto_framework);
        ensures exists<gas_schedule::GasScheduleV2>(@poto_framework);
        ensures exists<aggregator_factory::AggregatorFactory>(@poto_framework);
        ensures exists<coin::SupplyConfig>(@poto_framework);
        ensures exists<chain_id::ChainId>(@poto_framework);
        ensures exists<reconfiguration::Configuration>(@poto_framework);
        ensures exists<block::BlockResource>(@poto_framework);
        ensures exists<state_storage::StateStorageUsage>(@poto_framework);
        ensures exists<timestamp::CurrentTimeMicroseconds>(@poto_framework);
        ensures exists<account::Account>(@poto_framework);
        ensures exists<version::SetVersionCapability>(@poto_framework);
        ensures exists<staking_config::StakingConfig>(@poto_framework);
    }

    spec initialize_poto_coin {
        ...
        /// [high-level-req-3]
        requires !exists<stake::TopoCoinCapabilities>(@poto_framework);
        ensures exists<stake::TopoCoinCapabilities>(@poto_framework);
        requires exists<transaction_fee::TopoCoinCapabilities>(@poto_framework);
        ensures exists<transaction_fee::TopoCoinCapabilities>(@poto_framework);
    }
    ...
    spec create_initialize_validators_with_commission {
        ...
        include poto_coin::ExistsTopoCoin;
    }
    ...
    spec set_genesis_end {
        ...
        /// [high-level-req-4]
        requires len(global<stake::ValidatorSet>(@poto_framework).active_validators) >= 1;
        // property 5: The end of genesis should be marked on chain.
        /// [high-level-req-5]
        let addr = std::signer::address_of(poto_framework);
        aborts_if addr != @poto_framework;
        aborts_if exists<chain_status::GenesisEndMarker>(@poto_framework);
        ensures global<chain_status::GenesisEndMarker>(@poto_framework) == chain_status::GenesisEndMarker {};
    }

    spec schema InitalizeRequires {
        execution_config: vector<u8>;
        requires !exists<account::Account>(@poto_framework);
        requires chain_status::is_operating();
        requires len(execution_config) > 0;
        requires exists<staking_config::StakingRewardsConfig>(@poto_framework);
        requires exists<coin::CoinInfo<TopoCoin>>(@poto_framework);
        include CompareTimeRequires;
    }

    spec schema CompareTimeRequires {
        let staking_rewards_config = global<staking_config::StakingRewardsConfig>(@poto_framework);
