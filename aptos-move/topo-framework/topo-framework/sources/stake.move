...
module poto_framework::stake {
    ...
    struct ValidatorFees has key {
        fees_table: Table<address, Coin<TopoCoin>>
    }
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@poto_framework);
    ...
    public fun get_current_epoch_proposal_counts(
        ...
        let validator_performances =
            &borrow_global<ValidatorPerformance>(@poto_framework).validators;
    ...
    public fun get_pending_transaction_fee(): vector<u64> acquires PendingTransactionFee {
        ...
        let fee_table =
            &borrow_global<PendingTransactionFee>(@poto_framework).pending_fee_by_validator;
    ...
    public(friend) fun initialize(poto_framework: &signer) {
        ...
        move_to(poto_framework, ValidatorPerformance { validators: vector::empty() });
    }
    ...
    public(friend) fun store_poto_coin_mint_cap(
        poto_framework: &signer, mint_cap: MintCapability<TopoCoin>
    ) {
        system_addresses::assert_aptos_framework(poto_framework);
        move_to(poto_framework, TopoCoinCapabilities { mint_cap })
    }

    /// Allow on chain governance to remove validators from the validator set.
    public fun remove_validators(
        poto_framework: &signer, validators: &vector<address>
    ) acquires ValidatorSet {
        assert_reconfig_not_in_progress();
        system_addresses::assert_aptos_framework(poto_framework);
        let validator_set = borrow_global_mut<ValidatorSet>(@poto_framework);
    ...
    public fun initialize_pending_transaction_fee(framework: &signer) {
        ...
        if (!exists<PendingTransactionFee>(@poto_framework)) {
            move_to(
                framework,
    ...
    public fun set_transaction_fee_config(
        ...
        if (exists<TransactionFeeConfig>(@poto_framework)) {
            *borrow_global_mut<TransactionFeeConfig>(@poto_framework) = config;
        } else {
    ...
    public(friend) fun record_fee(
        ...
        let pending_fee = borrow_global_mut<PendingTransactionFee>(@poto_framework);
        let i = 0;
    ...
    public fun unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        coin::merge<TopoCoin>(&mut stake_pool.pending_inactive, unlocked_stake);
    ...
    public entry fun withdraw(
        ...
        coin::deposit<TopoCoin>(owner_address, coins);
    }
    ...
    public fun withdraw_with_cap(
    ...
    ): Coin<TopoCoin> acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (withdraw_amount == 0) return coin::zero<TopoCoin>();
    ...
    public(friend) fun update_performance_statistics(
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@poto_framework);
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires TopoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@poto_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@poto_framework);
        ...
        if (exists<PendingTransactionFee>(@poto_framework)) {
            let pending_fee_by_validator =
                &mut borrow_global_mut<PendingTransactionFee>(@poto_framework).pending_fee_by_validator;
            assert!(
    ...
    public fun next_validator_consensus_infos(): vector<ValidatorConsensusInfo> acquires ValidatorSet, ValidatorPerformance, StakePool, ValidatorConfig {
        let cur_validator_set = borrow_global<ValidatorSet>(@poto_framework);
        let staking_config = staking_config::get();
        let validator_perf = borrow_global<ValidatorPerformance>(@poto_framework);
    ...
    fun update_stake_pool(
    ...
    ) acquires TopoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
        ...
        let fee_limit =
            if (exists<TransactionFeeConfig>(@poto_framework)) {
                let TransactionFeeConfig::V0 { max_fee_octa_allowed_per_epoch_per_pool } =
                    borrow_global<TransactionFeeConfig>(@poto_framework);
        ...
        if (std::features::is_distribute_transaction_fee_enabled()) {
            let mint_cap =
                &borrow_global<TopoCoinCapabilities>(@poto_framework).mint_cap;
            if (fee_active > 0) {
    ...
    public fun configure_allowed_validators(
        poto_framework: &signer, accounts: vector<address>
    ) acquires AllowedValidators {
        let poto_framework_address = signer::address_of(poto_framework);
        system_addresses::assert_aptos_framework(poto_framework);
        if (!exists<AllowedValidators>(poto_framework_address)) {
            move_to(poto_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(poto_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@poto_framework)) { true }
        else {
            let allowed = borrow_global<AllowedValidators>(@poto_framework);
    ...
    use poto_std::bls12381::proof_of_possession_from_bytes;
    use poto_framework::reconfiguration_state;
    use poto_framework::validator_consensus_info;
    use poto_framework::validator_consensus_info::ValidatorConsensusInfo;
    #[test_only]
    ...
    public fun initialize_for_test(poto_framework: &signer) {
        reconfiguration_state::initialize(poto_framework);
    ...
    ) acquires TopoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration_state::initialize(poto_framework);
        if (!exists<ValidatorSet>(@poto_framework)) {
            initialize(poto_framework);
        };
        ...
        if (!exists<TopoCoinCapabilities>(@poto_framework)) {
            let (burn_cap, mint_cap) = poto_coin::initialize_for_test(poto_framework);
            store_poto_coin_mint_cap(poto_framework, mint_cap);
    ...
    #[test_only]
    public fun mint_coins(amount: u64): Coin<TopoCoin> acquires TopoCoinCapabilities {
        let mint_cap = &borrow_global<TopoCoinCapabilities>(@poto_framework).mint_cap;
        coin::mint(amount, mint_cap)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires TopoCoinCapabilities {
    ...
    ) acquires TopoCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool
    ) acquires AllowedValidators, TopoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        poto_framework: &signer, validator: &signer
    ) acquires AllowedValidators, TopoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(poto_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        poto_framework: &signer, validator_1: &signer, validator_2: &signer
    ) acquires AllowedValidators, TopoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        initialize_for_test_custom(
            poto_framework,
            50,
    ...
    #[test(poto_framework = @poto_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        ...
        assert!(coin::balance<TopoCoin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<TopoCoin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<TopoCoin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(poto_framework, 1);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).total_joining_power == 100, 0
        );
        ...
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).total_joining_power == 0, 1
        );
    }
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, validator_2 = @0x234)]
    public entry fun test_active_validator_can_withdraw_all_stake_and_rewards_at_once(
        poto_framework: &signer, validator: &signer, validator_2: &signer
    ) acquires AllowedValidators, TopoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        initialize_for_test(poto_framework);
        let (_sk, pk, pop) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<TopoCoin>(validator_address) == 1001, 2);
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).pending_inactive.borrow(0).addr ==
            validator_2_address,
            0
        );
        ...
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).pending_active.borrow(0).addr
                == validator_3_address,
            0
        );
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).active_validators.borrow(0).config
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        let vci_vec_0 =
            validator_consensus_infos_from_validator_set(
                borrow_global<ValidatorSet>(@poto_framework)
            );
        let vci_addrs =
            vci_vec_0.map_ref(|obj| {
        ...
        let vci_voting_powers =
            vci_vec_0.map_ref(|obj| {
                let vci: &ValidatorConsensusInfo = obj;
                validator_consensus_info::get_voting_power(vci)
            });
        assert!(
            vector[@0x5, @poto_framework, @0x3] == vci_addrs,
            1
        );
        assert!(
    ...
    public entry fun test_validator_rewards_are_performance_based(
        ...
        failed_proposer_indices.push_back(validator_2_index);
        ...
        let validator_2_index =
            borrow_global<ValidatorConfig>(validator_2_address).validator_index;
        failed_proposer_indices.push_back(validator_1_index);
        failed_proposer_indices.push_back(validator_2_index);
    ...
    public entry fun test_update_performance_statistics_should_not_fail_due_to_out_of_bounds(
        ...
        let failed_proposer_indices = vector::empty<u64>();
        failed_proposer_indices.push_back(valid_validator_index);
        failed_proposer_indices.push_back(out_of_bounds_index);
        update_performance_statistics(option::none(), failed_proposer_indices);
        end_epoch();
    ...
    public entry fun test_invalid_config(
        poto_framework: &signer, validator: &signer
    ) acquires AllowedValidators, TopoCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(
            poto_framework,
            50,
            10000,
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).active_validators.length()
                == 2,
            0
        );
        ...
        remove_validators(poto_framework, &vector[validator_to_remove]);
        assert!(
            borrow_global<ValidatorSet>(@poto_framework).active_validators.length()
                == 1,
            0
    ...
    #[test(
        vm = @0x0, poto_framework = @0x1, validator_0 = @0x123, validator_1 = @0x234
    )]
    public entry fun test_transaction_fee(
        ...
        initialize_for_test(poto_framework);
        initialize_pending_transaction_fee(poto_framework);
        features::change_feature_flags_for_testing(
            poto_framework,
    ...
    public entry fun test_transaction_fee_limit(
...

        {
            ...
            set_transaction_fee_config(poto_framework, config);
            end_epoch();
        ...
        let config =
            TransactionFeeConfig::V0 { max_fee_octa_allowed_per_epoch_per_pool: 200 };
        set_transaction_fee_config(poto_framework, config);
    }

    #[test_only]
    public fun end_epoch() acquires TopoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(poto_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        poto_framework: &signer, validator: &signer
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        let (burn, mint) = poto_coin::initialize_for_test(poto_framework);
        configure_allowed_validators(poto_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(poto_framework, vector[]);
        let (burn, mint) = poto_coin::initialize_for_test(poto_framework);
    ...
    public entry fun test_validator_set_liveness_fallback(
        ...
        initialize_test_validator(&pk, &pop, validator, 100, true, true);
        staking_config::update_required_stake(poto_framework, 1000, 10000);
        end_epoch();

        // Verify that ValidatorSetLivenessFallback event was emitted
        let validator_set = &ValidatorSet[@poto_framework];
        let (minimum_stake, _) =
            staking_config::get_required_stake(&staking_config::get());
