...
module poto_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::features;
    use std::signer;
    ...
    use poto_framework::account::{Self, SignerCapability};
    ...
    use poto_framework::stake::{Self, OwnerCapability};
    use poto_framework::staking_config;

    const SALT: vector<u8> = b"poto_framework::staking_contract";
    ...
    #[
        resource_group_member(
            group = poto_framework::staking_contract::StakingGroupContainer
        )
    ]
    ...
    fun distribute_internal(
        ...
        if (coin::value(&coins) > 0) {
            poto_account::deposit_coins(staker, coins);
    ...
    public fun setup_staking_contract(
        ...
        setup(poto_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
    ...
    #[test(poto_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(
        poto_framework: &signer, staker: &signer, operator: &signer
        ...
        stake::mint(staker, INITIAL_BALANCE);
        let previous_principal = last_recorded_principal(
            staker_address, operator_address
        );
        ...
        unpaid_commission =
            with_rewards(unpaid_commission) + commission_on_withdrawn_amount;
        withdrawn_amount = with_rewards(withdrawn_amount)
            - commission_on_withdrawn_amount;
        stake::assert_stake_pool(
            pool_address,
            0,
            with_rewards(new_balance),
            0,
            0
        );
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(poto_framework, staker, operator, 50, 100);
    }
    ...
    public entry fun test_operator_can_set_beneficiary(
        ...
        poto_framework::poto_account::create_account(beneficiary_address);
    ...
    public entry fun test_staker_can_withdraw_partial_stake(
        ...
        unpaid_commission += commission_on_withdrawn_stake;
    ...
    public entry fun test_multiple_distributions_added_before_distribute(
        ...
        unpaid_commission =
            with_rewards(unpaid_commission) + commission_on_withdrawn_stake
        ...
        new_balance -= new_withdrawn_stake;
        withdrawn_stake =
            with_rewards(withdrawn_stake) - commission_on_withdrawn_stake
                + new_withdrawn_stake;
        stake::assert_stake_pool(
            pool_address,
            new_balance,
    ...
    public fun assert_no_pending_distributions(
        ...
        let shareholders_count = distribution_pool.shareholders_count();
        assert!(shareholders_count == 0, shareholders_count);
        let total_coins_remaining = distribution_pool.total_coins();
