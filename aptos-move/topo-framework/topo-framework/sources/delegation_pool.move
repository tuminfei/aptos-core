 ...
 */
module poto_framework::delegation_pool {
    use std::error;
    use std::features;
    use std::signer;
    use std::vector;
    ...
    use poto_std::math64;
    use poto_std::pool_u64_unbound as pool_u64;
    use poto_std::table::{Self, Table};
    use poto_std::smart_table::{Self, SmartTable};

    use poto_framework::account;
    use poto_framework::poto_account;
    use poto_framework::poto_coin::TopoCoin;
    use poto_framework::poto_governance;
    use poto_framework::coin;
    use poto_framework::event::{Self, EventHandle, emit};
    use poto_framework::permissioned_signer;
    use poto_framework::stake;
    use poto_framework::stake::get_operator;
    use poto_framework::staking_config;
    use poto_framework::timestamp;

    const MODULE_SALT: vector<u8> = b"poto_framework::delegation_pool";
    ...
    public fun calculate_and_update_remaining_voting_power(
        ...
        if (poto_governance::get_remaining_voting_power(pool_address, proposal_id) == 0) {
            return 0
        };
    ...
    public entry fun vote(
        ...
        poto_governance::assert_proposal_expiration(pool_address, proposal_id);
        ...
        *used_voting_power += voting_power;
        ...
        poto_governance::partial_vote(&pool_signer, pool_address, proposal_id, voting_power, should_pass);
    ...
    public entry fun create_proposal(
        ...
        assert!(
            total_voting_power >= poto_governance::get_required_proposer_stake(),
            error::invalid_argument(EINSUFFICIENT_PROPOSER_STAKE));
        let pool_signer = retrieve_stake_pool_owner(borrow_global<DelegationPool>(pool_address));
        let proposal_id = poto_governance::create_proposal_v2_impl(
            &pool_signer,
            pool_address,
            execution_hash,
            metadata_location,
    ...
    public entry fun add_stake(
        ...
        poto_account::transfer(delegator, pool_address, amount);
    ...
    fun withdraw_internal(
        ...
        poto_account::transfer(stake_pool_owner, delegator_address, amount);
    ...
    inline fun assert_and_update_proposal_used_voting_power(
        ...
        let stake_pool_remaining_voting_power = poto_governance::get_remaining_voting_power(pool_address, proposal_id);
        let stake_pool_used_voting_power = poto_governance::get_voting_power(
            pool_address
    ...
    #[test_only]
    use poto_std::fixed_point64;
    #[test_only]
    use poto_framework::stake::fast_forward_to_unlock;
    #[test_only]
    use poto_framework::timestamp::fast_forward_seconds;
    ...
    #[test_only]
    public fun initialize_for_test(poto_framework: &signer) {
    ...
    #[test_only]
    public fun initialize_for_test_no_reward(poto_framework: &signer) {
        initialize_for_test_custom(
            poto_framework,
            100 * ONE_TOPO,
            10000000 * ONE_TOPO,
    ...
    public fun initialize_for_test_custom(
        ...
        account::create_account_for_test(signer::address_of(poto_framework));
        ...
        reconfiguration::initialize_for_test(poto_framework);
    ...
    #[test_only]
    public fun enable_delegation_pool_allowlisting_feature(poto_framework: &signer) {
        features::change_feature_flags_for_testing(
            poto_framework,
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123)]
    public entry fun test_set_operator(
        poto_framework: &signer,
        validator: &signer,
    ) acquires DelegationPoolOwnership, DelegationPool, GovernanceRecords, BeneficiaryForOperator, NextCommissionPercentage {
        initialize_for_test(poto_framework);
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020)]
    public entry fun test_add_stake_fee(
        ...
        assert!(get_add_stake_fee(pool_address, 1000000 * ONE_TOPO) == 0, 0);

        // add 1M TOPO, join the validator set and activate this stake
        stake::mint(validator, 1000000 * ONE_TOPO);
        add_stake(validator, pool_address, 1000000 * ONE_TOPO);
        ...
        assert!(get_add_stake_fee(pool_address, 100000 * ONE_TOPO) == 62259941466, 0);

        // add pending_active stake from multiple delegators
        stake::mint(delegator1, 100000 * ONE_TOPO);
        add_stake(delegator1, pool_address, 100000 * ONE_TOPO);
        stake::mint(delegator2, 10000 * ONE_TOPO);
        add_stake(delegator2, pool_address, 10000 * ONE_TOPO);
        ...
        stake::mint(delegator1, 10000 * ONE_TOPO);
        let (delegator1_active, _, _) = get_stake(pool_address, delegator1_address);
        add_stake(delegator1, pool_address, 10000 * ONE_TOPO);

        let fee = get_add_stake_fee(pool_address, 10000 * ONE_TOPO);
        assert_delegation(delegator1_address, pool_address, delegator1_active + 10000 * ONE_TOPO - fee, 0, 0);

        // delegator 2 should not benefit in any way from this new stake
        assert_delegation(delegator2_address, pool_address, 1000000000000, 0, 0);

        // add more stake from delegator 2
        stake::mint(delegator2, 100000 * ONE_TOPO);
        add_stake(delegator2, pool_address, 100000 * ONE_TOPO);
        ...
        // add more stake from delegator 1
        stake::mint(delegator1, 20000 * ONE_TOPO);
        (delegator1_active, _, _) = get_stake(pool_address, delegator1_address);
        add_stake(delegator1, pool_address, 20000 * ONE_TOPO);

        fee = get_add_stake_fee(pool_address, 20000 * ONE_TOPO);
        assert_delegation(delegator1_address, pool_address, delegator1_active + 20000 * ONE_TOPO - fee, 0, 0);

        // delegator 1 unlocks his entire newly added stake
        unlock(delegator1, pool_address, 20000 * ONE_TOPO - fee);
        end_aptos_epoch();
        ...
        assert_delegation(
            delegator1_address,
            pool_address,
            delegator1_active + 20000 * ONE_TOPO - fee,
            0,
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, delegator = @0x010)]
    public entry fun test_never_create_pending_withdrawal_if_no_shares_bought(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 1000 * ONE_TOPO, true, false);
        ...
        stake::mint(delegator, 10 * ONE_TOPO);
        add_stake(delegator, pool_address, 10 * ONE_TOPO);
        end_aptos_epoch();

        unlock(validator, pool_address, 100 * ONE_TOPO);
    ...
    public entry fun test_add_stake_single(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 1000 * ONE_TOPO, false, false);

        let validator_address = signer::address_of(validator);
        let pool_address = get_owned_pool_address(validator_address);

        // validator is inactive => added stake is `active` by default
        stake::assert_stake_pool(pool_address, 1000 * ONE_TOPO, 0, 0, 0);
        assert_delegation(validator_address, pool_address, 1000 * ONE_TOPO, 0, 0);

        // zero `add_stake` fee as validator is not producing rewards this epoch
        assert!(get_add_stake_fee(pool_address, 250 * ONE_TOPO) == 0, 0);

        // check `add_stake` increases `active` stakes of delegator and stake pool
        stake::mint(validator, 300 * ONE_TOPO);
        let balance = coin::balance<TopoCoin>(validator_address);
        add_stake(validator, pool_address, 250 * ONE_TOPO);

        // check added stake have been transferred out of delegator account
        assert!(coin::balance<TopoCoin>(validator_address) == balance - 250 * ONE_TOPO, 0);
        // zero `add_stake` fee charged from added stake
        assert_delegation(validator_address, pool_address, 1250 * ONE_TOPO, 0, 0);
        ...
        stake::assert_stake_pool(pool_address, 1250 * ONE_TOPO, 0, 0, 0);
        ...
        stake::mint(validator, 250 * ONE_TOPO);
        add_stake(validator, pool_address, 250 * ONE_TOPO);

        let fee1 = get_add_stake_fee(pool_address, 250 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 1500 * ONE_TOPO - fee1, 0, 0);
        // check `add_stake` fee has been transferred to the null shareholder
        assert_delegation(NULL_SHAREHOLDER, pool_address, fee1, 0, 0);
        stake::assert_stake_pool(pool_address, 1250 * ONE_TOPO, 0, 250 * ONE_TOPO, 0);

        // add 100 additional coins being pending_active until next epoch
        stake::mint(validator, 100 * ONE_TOPO);
        add_stake(validator, pool_address, 100 * ONE_TOPO);

        let fee2 = get_add_stake_fee(pool_address, 100 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 1600 * ONE_TOPO - fee1 - fee2, 0, 0);
        // check `add_stake` fee has been transferred to the null shareholder
        assert_delegation(NULL_SHAREHOLDER, pool_address, fee1 + fee2, 0, 0);
        stake::assert_stake_pool(pool_address, 1250 * ONE_TOPO, 0, 350 * ONE_TOPO, 0);
        ...
        stake::mint(validator, 200 * ONE_TOPO);
        add_stake(validator, pool_address, 200 * ONE_TOPO);

        fee1 = get_add_stake_fee(pool_address, 200 * ONE_TOPO);
    ...
    public entry fun test_add_stake_many(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 1000 * ONE_TOPO, true, true);
        ...
        stake::mint(delegator, 250 * ONE_TOPO);
        add_stake(delegator, pool_address, 250 * ONE_TOPO);

        let fee1 = get_add_stake_fee(pool_address, 250 * ONE_TOPO);
        assert_delegation(delegator_address, pool_address, 250 * ONE_TOPO - fee1, 0, 0);
        assert_delegation(validator_address, pool_address, 1000 * ONE_TOPO, 0, 0);
        stake::assert_stake_pool(pool_address, 1000 * ONE_TOPO, 0, 250 * ONE_TOPO, 0);

        end_aptos_epoch();
        // 1000 * 1.01 active stake + 250 pending_active stake
        stake::assert_stake_pool(pool_address, 1260 * ONE_TOPO, 0, 0, 0);
        // delegator got its `add_stake` fee back
        assert_delegation(delegator_address, pool_address, 250 * ONE_TOPO, 0, 0);
        ...
        fee1 = get_add_stake_fee(pool_address, 250 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 125999999999 - fee1, 0, 0);
        assert_delegation(delegator_address, pool_address, 250 * ONE_TOPO, 0, 0);
        stake::assert_stake_pool(pool_address, 1260 * ONE_TOPO, 0, 250 * ONE_TOPO, 0);

        // add another 100 coins from second account
        stake::mint(delegator, 100 * ONE_TOPO);
        add_stake(delegator, pool_address, 100 * ONE_TOPO);

        let fee2 = get_add_stake_fee(pool_address, 100 * ONE_TOPO);
        assert_delegation(delegator_address, pool_address, 350 * ONE_TOPO - fee2, 0, 0);
        assert_delegation(validator_address, pool_address, 125999999999 - fee1, 0, 0);
        stake::assert_stake_pool(pool_address, 1260 * ONE_TOPO, 0, 350 * ONE_TOPO, 0);
    ...
    public entry fun test_unlock_single(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 100 * ONE_TOPO, true, true);
        ...
        let fee = get_add_stake_fee(pool_address, 200 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 300 * ONE_TOPO - fee, 0, 0);
        stake::assert_stake_pool(pool_address, 100 * ONE_TOPO, 0, 200 * ONE_TOPO, 0);

        // cannot unlock pending_active stake (only 100/300 stake can be displaced)
        unlock(validator, pool_address, 100 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 200 * ONE_TOPO - fee, 0, 100 * ONE_TOPO);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 100 * ONE_TOPO);
        stake::assert_stake_pool(pool_address, 0, 0, 200 * ONE_TOPO, 100 * ONE_TOPO);
        assert_inactive_shares_pool(pool_address, 0, true, 100 * ONE_TOPO);

        // reactivate entire pending_inactive stake progressively
        reactivate_stake(validator, pool_address, 50 * ONE_TOPO);

        assert_delegation(validator_address, pool_address, 250 * ONE_TOPO - fee, 0, 50 * ONE_TOPO);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 50 * ONE_TOPO);
        stake::assert_stake_pool(pool_address, 50 * ONE_TOPO, 0, 200 * ONE_TOPO, 50 * ONE_TOPO);
        ...
        assert_delegation(validator_address, pool_address, 301 * ONE_TOPO, 0, 0);
        stake::assert_stake_pool(pool_address, 301 * ONE_TOPO, 0, 0, 0);

        // can unlock more than at previous epoch as the pending_active stake became active
        unlock(validator, pool_address, 150 * ONE_TOPO);
        ...
        stake::mint(delegator, 50 * ONE_TOPO);
        add_stake(delegator, pool_address, 50 * ONE_TOPO);

        // observed lockup cycle should have advanced at `add_stake`(on synchronization)
        assert!(observed_lockup_cycle(pool_address) == 1, 0);

        fee = get_add_stake_fee(pool_address, 50 * ONE_TOPO);
        assert_delegation(delegator_address, pool_address, 4999999999 - fee, 0, 0);
        assert_delegation(validator_address, pool_address, 15403510001, 15301499997, 0);
        stake::assert_stake_pool(pool_address, 15403510001, 15301499997, 50 * ONE_TOPO, 0);

        // cannot withdraw stake unlocked by others
        withdraw(delegator, pool_address, 50 * ONE_TOPO);
        assert!(coin::balance<TopoCoin>(delegator_address) == 0, 0);

        // withdraw own unlocked stake
        withdraw(validator, pool_address, 15301499997);
        assert!(coin::balance<TopoCoin>(validator_address) == 15301499997, 0);
        ...
        assert!(coin::balance<TopoCoin>(validator_address) == balance + 5457545100, 0);
        ...
        initialize_test_validator(delegator, 100 * ONE_TOPO, true, true);
        ...
        balance = coin::balance<TopoCoin>(validator_address);
        withdraw(validator, pool_address, 10303010000);

        assert!(coin::balance<TopoCoin>(validator_address) == balance + 10303010000, 0);
        ...
        stake::mint(validator, 30 * ONE_TOPO);
        add_stake(validator, pool_address, 30 * ONE_TOPO);
        unlock(validator, pool_address, 10 * ONE_TOPO);
        ...
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert!(coin::balance<TopoCoin>(validator_address) == balance + 1000000000, 0);
    }
    ...
    public entry fun test_total_coins_inactive(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 200 * ONE_TOPO, true, true);
        ...
        stake::mint(delegator1, 100 * ONE_TOPO);
        stake::mint(delegator2, 200 * ONE_TOPO);
        add_stake(delegator1, pool_address, 100 * ONE_TOPO);
        add_stake(delegator2, pool_address, 200 * ONE_TOPO);
        end_aptos_epoch();

        assert_delegation(delegator1_address, pool_address, 100 * ONE_TOPO, 0, 0);
        assert_delegation(delegator2_address, pool_address, 200 * ONE_TOPO, 0, 0);

        // unlock some stake from delegator 1
        unlock(delegator1, pool_address, 50 * ONE_TOPO);
        ...
        assert_delegation(delegator2_address, pool_address, 202 * ONE_TOPO, 0, 0);

        synchronize_delegation_pool(pool_address);
        assert!(total_coins_inactive(pool_address) == 5049999998, 0);

        // unlock some stake from delegator 2
        unlock(delegator2, pool_address, 50 * ONE_TOPO);
        ...
        unlock(validator, pool_address, 50 * ONE_TOPO);

        // create dummy validator to ensure the existing validator can leave the set
        initialize_test_validator(delegator1, 100 * ONE_TOPO, true, true);
        // inactivate validator
        stake::leave_validator_set(validator, pool_address);
        end_aptos_epoch();
    ...
    public entry fun test_reactivate_stake_single(
        ...
        unlock(validator, pool_address, 100 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 100 * ONE_TOPO, 0, 100 * ONE_TOPO);
        stake::assert_stake_pool(pool_address, 100 * ONE_TOPO, 0, 0, 100 * ONE_TOPO);
        assert_pending_withdrawal(validator_address, pool_address, true, 0, false, 100 * ONE_TOPO);

        // add some stake to pending_active state
        stake::mint(validator, 150 * ONE_TOPO);
        add_stake(validator, pool_address, 150 * ONE_TOPO);

        let fee = get_add_stake_fee(pool_address, 150 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 250 * ONE_TOPO - fee, 0, 100 * ONE_TOPO);
        stake::assert_stake_pool(pool_address, 100 * ONE_TOPO, 0, 150 * ONE_TOPO, 100 * ONE_TOPO);

        // can reactivate only pending_inactive stake
        reactivate_stake(validator, pool_address, 150 * ONE_TOPO);

        assert_delegation(validator_address, pool_address, 350 * ONE_TOPO - fee, 0, 0);
        stake::assert_stake_pool(pool_address, 200 * ONE_TOPO, 0, 150 * ONE_TOPO, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        ...
        unlock(validator, pool_address, 100 * ONE_TOPO);
        assert!(coin::balance<TopoCoin>(validator_address) == 15149999998, 0);
    ...
    public entry fun test_withdraw_many(
        ...
        stake::mint(delegator, 200 * ONE_TOPO);
        add_stake(delegator, pool_address, 200 * ONE_TOPO);
        ...
        assert_delegation(delegator_address, pool_address, 200 * ONE_TOPO, 0, 0);
        ...
        unlock(delegator, pool_address, 100 * ONE_TOPO);
        ...
        let balance = coin::balance<TopoCoin>(validator_address);
        withdraw(validator, pool_address, MAX_U64);
        assert!(coin::balance<TopoCoin>(validator_address) == balance + 10100000000, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert_inactive_shares_pool(pool_address, 0, false, 0);
    ...
    public entry fun test_inactivate_no_excess_stake(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 1200 * ONE_TOPO, true, true);
        ...
        unlock(validator, pool_address, 200 * ONE_TOPO);
    ...
    public entry fun test_active_stake_rewards(
        ...
        assert_delegation(validator_address, pool_address, 1010 * ONE_TOPO, 0, 0);
        ...
        let fee = get_add_stake_fee(pool_address, 200 * ONE_TOPO);
        assert_delegation(validator_address, pool_address, 1210 * ONE_TOPO - fee, 0, 0);
        ...
        // add stake in pending_active state
        stake::mint(validator, 1000 * ONE_TOPO);
        add_stake(validator, pool_address, 1000 * ONE_TOPO);

        fee = get_add_stake_fee(pool_address, 1000 * ONE_TOPO);
    ...
    public entry fun test_active_stake_rewards_multiple(
        ...
        // add stake in pending_active state
        stake::mint(delegator, 300 * ONE_TOPO);
        add_stake(delegator, pool_address, 300 * ONE_TOPO);

        let fee = get_add_stake_fee(pool_address, 300 * ONE_TOPO);
        assert_delegation(delegator_address, pool_address, 300 * ONE_TOPO - fee, 0, 0);
        assert_delegation(validator_address, pool_address, 200 * ONE_TOPO, 0, 0);
        stake::assert_stake_pool(pool_address, 200 * ONE_TOPO, 0, 300 * ONE_TOPO, 0);

        end_aptos_epoch();
        // `delegator` got its `add_stake` fee back and `validator` its active stake rewards
        assert_delegation(delegator_address, pool_address, 300 * ONE_TOPO, 0, 0);
        assert_delegation(validator_address, pool_address, 20199999999, 0, 0);
        stake::assert_stake_pool(pool_address, 502 * ONE_TOPO, 0, 0, 0);

        // delegators earn their own rewards from now on
        end_aptos_epoch();
        assert_delegation(delegator_address, pool_address, 303 * ONE_TOPO, 0, 0);
        ...
        stake::mint(delegator, 1000 * ONE_TOPO);
        add_stake(delegator, pool_address, 1000 * ONE_TOPO);
    ...
    public entry fun test_pending_inactive_stake_rewards(
        ...
        unlock(validator, pool_address, 200 * ONE_TOPO);
        assert!(coin::balance<TopoCoin>(validator_address) == 20606019996, 0);
    ...
    public entry fun test_out_of_order_redeem(
        ...
        stake::mint(delegator1, 300 * ONE_TOPO);
        add_stake(delegator1, pool_address, 300 * ONE_TOPO);

        stake::mint(delegator2, 300 * ONE_TOPO);
        add_stake(delegator2, pool_address, 300 * ONE_TOPO);

        end_aptos_epoch();

        // create the pending withdrawal of delegator 1 in lockup cycle 0
        unlock(delegator1, pool_address, 150 * ONE_TOPO);
        ...
        unlock(delegator2, pool_address, 150 * ONE_TOPO);
        ...
        assert!(coin::balance<TopoCoin>(delegator1_address) == 15149999998, 0);
        assert!(coin::balance<TopoCoin>(delegator2_address) == 5149999997, 0);

        // recreate the pending withdrawal of delegator 1 in lockup cycle 2
        unlock(delegator1, pool_address, 100 * ONE_TOPO);
        ...
        assert!(coin::balance<TopoCoin>(delegator2_address) == 15149999998, 0);
        ...
        assert!(coin::balance<TopoCoin>(delegator1_address) == 15149999998 + 10099999998, 0);
        assert_pending_withdrawal(delegator1_address, pool_address, false, 0, false, 0);
    }
    ...
    public entry fun test_operator_fee(
        ...
        stake::assert_stake_pool(pool_address, 300 * ONE_TOPO, 0, 0, 0);
        ...
        unlock(delegator2, pool_address, 100 * ONE_TOPO);
        ...
        stake::mint(delegator1, 100 * ONE_TOPO);
        assert!(get_add_stake_fee(pool_address, 100 * ONE_TOPO) > 0, 0);
        add_stake(delegator1, pool_address, 100 * ONE_TOPO);
        ...
        unlock(delegator2, pool_address, 100 * ONE_TOPO);
        // 10807241561 - 100 TOPO < `MIN_COINS_ON_SHARES_POOL` thus active stake is entirely unlocked
        ...
        coin::register<TopoCoin>(validator);
        synchronize_delegation_pool(pool_address);
        assert!(coin::balance<TopoCoin>(validator_address) == 25536996, 0);
    ...
    #[test(poto_framework = @poto_framework, old_operator = @0x123, delegator = @0x010, new_operator = @0x020)]
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        poto_account::create_account(operator1_address);

        let operator2_address = signer::address_of(operator2);
        poto_account::create_account(operator2_address);

        let beneficiary_address = signer::address_of(beneficiary);
        poto_account::create_account(beneficiary_address);
        ...
        stake::mint(delegator, 2000000 * ONE_TOPO);
        add_stake(delegator, pool_address, 2000000 * ONE_TOPO);
        unlock(delegator, pool_address, 1000000 * ONE_TOPO);
        ...
        withdraw(operator1, pool_address, ONE_TOPO);
        assert!(coin::balance<TopoCoin>(operator1_address) == ONE_TOPO - 1, 0);
        ...
        unlock(beneficiary, pool_address, ONE_TOPO);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_aptos_epoch();

        withdraw(beneficiary, pool_address, ONE_TOPO);
        assert!(coin::balance<TopoCoin>(beneficiary_address) == ONE_TOPO - 1, 0);
        assert!(coin::balance<TopoCoin>(operator1_address) == ONE_TOPO - 1, 0);
        ...
        end_aptos_epoch();
        unlock(operator2, pool_address, ONE_TOPO);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_aptos_epoch();

        withdraw(operator2, pool_address, ONE_TOPO);
        assert!(coin::balance<TopoCoin>(beneficiary_address) == ONE_TOPO - 1, 0);
        assert!(coin::balance<TopoCoin>(operator2_address) == ONE_TOPO - 1, 0);
    }

    #[test(poto_framework = @poto_framework, operator = @0x123, delegator = @0x010)]
    public entry fun test_update_commission_percentage(
        ...
        assert!(get_add_stake_fee(pool_address, 100 * ONE_TOPO) == 76756290, 0);
    ...
    public entry fun test_min_stake_is_preserved(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 100 * ONE_TOPO, true, false);
        ...
        // add stake without fees as validator is not active yet
        stake::mint(delegator1, 50 * ONE_TOPO);
        add_stake(delegator1, pool_address, 50 * ONE_TOPO);
        stake::mint(delegator2, 16 * ONE_TOPO);
        add_stake(delegator2, pool_address, 16 * ONE_TOPO);
        ...
        assert_delegation(delegator2_address, pool_address, 16 * ONE_TOPO, 0, 0);
        unlock(delegator2, pool_address, 1);
        assert_delegation(delegator2_address, pool_address, 0, 0, 16 * ONE_TOPO);
        reactivate_stake(delegator2, pool_address, 1);
        assert_delegation(delegator2_address, pool_address, 16 * ONE_TOPO, 0, 0);

        unlock(delegator2, pool_address, ONE_TOPO);
        assert_delegation(delegator2_address, pool_address, 0, 0, 16 * ONE_TOPO);
        reactivate_stake(delegator2, pool_address, 2 * ONE_TOPO);
        assert_delegation(delegator2_address, pool_address, 16 * ONE_TOPO, 0, 0);
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, delegator1 = @0x010)]
    ...
    public entry fun test_create_proposal_abort_if_inefficient_stake(
        ...
        initialize_for_test(poto_framework);
        poto_governance::initialize_for_test(
            poto_framework,
            (10 * ONE_TOPO as u128),
            100 * ONE_TOPO,
            1000,
        ...
        account::create_account_for_test(delegator1_address);
        stake::mint(delegator1, 100 * ONE_TOPO);
        add_stake(delegator1, pool_address, 10 * ONE_TOPO);
        end_aptos_epoch();
    ...
    public entry fun test_voting_power_change(
        ...
        initialize_for_test_no_reward(poto_framework);
        poto_governance::initialize_for_test(
            poto_framework,
            (10 * ONE_TOPO as u128),
            100 * ONE_TOPO,
        ...
        stake::mint(delegator1, 110 * ONE_TOPO);
        add_stake(delegator1, pool_address, 10 * ONE_TOPO);
        stake::mint(delegator2, 110 * ONE_TOPO);
        add_stake(delegator2, pool_address, 90 * ONE_TOPO);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 10 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 90 * ONE_TOPO, 1);
        ...
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_aptos_epoch();
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 10 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 10 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);

        // delegator1 adds stake to the pool. Voting power changes immediately.
        add_stake(delegator1, pool_address, 90 * ONE_TOPO);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
        ...
        unlock(delegator1, pool_address, 90 * ONE_TOPO);
        delegate_voting_power(delegator1, pool_address, voter2_address);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 100 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 90 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        ...
        withdraw(delegator1, pool_address, 45 * ONE_TOPO);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 100 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);

        // voter2 adds stake for itself. Voting power changes immediately.
        stake::mint(voter2, 110 * ONE_TOPO);
        add_stake(voter2, pool_address, 10 * ONE_TOPO);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 110 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
    }
    ...
    public entry fun test_voting_power_change_for_rewards(
        ...
        // 50% commission rate
        initialize_test_validator_custom(validator, 100 * ONE_TOPO, true, false, 5000);
        ...
        add_stake(delegator2, pool_address, 90 * ONE_TOPO);
        // By default, the voter of a delegator is itself.
        assert!(calculate_and_update_voter_total_voting_power(pool_address, validator_address) == 100 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 10 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 90 * ONE_TOPO, 1);
        ...
        end_aptos_epoch();
        end_aptos_epoch();
        assert!(calculate_and_update_voter_total_voting_power(pool_address, validator_address) == 550 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 25 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 225 * ONE_TOPO, 1);
        ...
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 375 * ONE_TOPO, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, voter2_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator1_address) == 0, 1);
        assert!(calculate_and_update_voter_total_voting_power(pool_address, delegator2_address) == 0, 1);
    }
    ...
    public entry fun test_delegate_voting_power_applies_next_lockup(
        ...
        stake::mint(delegator, 100 * ONE_TOPO);
        add_stake(delegator, pool_address, 20 * ONE_TOPO);
        ...
        assert!(
            calculate_and_update_voter_total_voting_power(
                pool_address,
                delegator_address
            ) == 20 * ONE_TOPO - get_add_stake_fee(pool_address, 20 * ONE_TOPO),
            0
        );
        ...
        assert!(
            calculate_and_update_voter_total_voting_power(pool_address, voter1_address) == 20 * ONE_TOPO,
            0
        );
        ...
        add_stake(delegator, pool_address, 30 * ONE_TOPO);
        assert!(
            calculate_and_update_voter_total_voting_power(
                pool_address,
                voter1_address
            ) == 20 * ONE_TOPO + 30 * ONE_TOPO - get_add_stake_fee(pool_address, 30 * ONE_TOPO),
            0
        );
    ...
    public entry fun test_delegate_voting_power_from_inactive_validator(
        ...
        initialize_test_validator(validator, 100 * ONE_TOPO, true, false);
        initialize_test_validator(validator_min_consensus, 100 * ONE_TOPO, true, true);
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, delegator_1 = @0x010)]
    #[expected_failure(abort_code = 0x30018, location = Self)]
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123, delegator_1 = @0x010, delegator_2 = @0x020)]
    public entry fun test_allowlist_operations_only_e2e(
        ...
        initialize_for_test(poto_framework);
        initialize_test_validator(validator, 100 * ONE_TOPO, true, true);
        enable_delegation_pool_allowlisting_feature(poto_framework);
    ...
    public entry fun test_cannot_evict_null_address(
        ...
        stake::mint(delegator_1, 50 * ONE_TOPO);
        add_stake(delegator_1, pool_address, 50 * ONE_TOPO);
    ...
    public entry fun test_cannot_add_stake_if_not_allowlisted(
        ...
        stake::mint(delegator_1, 30 * ONE_TOPO);
        add_stake(delegator_1, pool_address, 20 * ONE_TOPO);

        end_aptos_epoch();
        assert_delegation(delegator_1_address, pool_address, 20 * ONE_TOPO, 0, 0);
        ...
        add_stake(delegator_1, pool_address, 10 * ONE_TOPO);
    }
    ...
    public entry fun test_cannot_reactivate_stake_if_not_allowlisted(
        ...
        end_aptos_epoch();
        assert_delegation(delegator_1_address, pool_address, 50 * ONE_TOPO, 0, 0);

        // some of the stake is unlocked by the delegator
        unlock(delegator_1, pool_address, 30 * ONE_TOPO);
        assert_delegation(delegator_1_address, pool_address, 20 * ONE_TOPO, 0, 2999999999);
        ...
        reactivate_stake(delegator_1, pool_address, 50 * ONE_TOPO);
        assert_delegation(delegator_1_address, pool_address, 0, 0, 4999999999);
    }
    ...
    public entry fun test_delegation_pool_allowlisting_e2e(
        ...
        assert!(!allowlisting_enabled(pool_address), 0);
        stake::mint(delegator_1, 100 * ONE_TOPO);
        stake::mint(delegator_2, 100 * ONE_TOPO);
        add_stake(delegator_1, pool_address, 50 * ONE_TOPO);
        add_stake(delegator_2, pool_address, 30 * ONE_TOPO);

        end_aptos_epoch();
        assert_delegation(delegator_1_address, pool_address, 50 * ONE_TOPO, 0, 0);
        assert_delegation(delegator_2_address, pool_address, 30 * ONE_TOPO, 0, 0);
        ...
        assert_delegation(delegator_2_address, pool_address, 0, 0, 30 * ONE_TOPO);
        ...
        let active = borrow_global<DelegationPool>(pool_address).active_shares.balance(delegator_1_address) + get_add_stake_fee(pool_address, 10 * ONE_TOPO);
        ...
        assert_delegation(delegator_1_address, pool_address, 20 * ONE_TOPO, 0, 6223120049);

        // can reactivate stake when allowlisted
        reactivate_stake(delegator_1, pool_address, 5223120050);
        assert_delegation(delegator_1_address, pool_address, 20 * ONE_TOPO + 5223120049, 0, 10 * ONE_TOPO);
