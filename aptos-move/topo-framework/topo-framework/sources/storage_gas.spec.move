spec poto_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@poto_framework);
    }
    ...
    spec set_config(poto_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(poto_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@poto_framework);
        aborts_if exists<StorageGas>(@poto_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@poto_framework);
        ensures exists<StorageGas>(@poto_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@poto_framework);
        aborts_if !exists<StorageGas>(@poto_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@poto_framework);
    }
