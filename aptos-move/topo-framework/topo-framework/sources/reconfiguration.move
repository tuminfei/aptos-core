...
module poto_framework::reconfiguration {
    use std::error;
    use std::signer;
    ...
    friend poto_framework::consensus_config;
    friend poto_framework::execution_config;
    friend poto_framework::gas_schedule;
    friend poto_framework::genesis;
    friend poto_framework::version;
    ...
    public(friend) fun initialize(poto_framework: &signer) {
        ...
        assert!(
            account::get_guid_next_creation_num(signer::address_of(poto_framework)) == 2,
        ...
        move_to<Configuration>(
            ...
            Configuration {
                ...
                events: account::new_event_handle<NewEpochEvent>(poto_framework)
            }
        );
    }
    ...
    fun disable_reconfiguration(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(poto_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(poto_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_aptos_framework(poto_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(
            signer::address_of(poto_framework)
        );
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@poto_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@poto_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@poto_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@poto_framework).epoch
    }
