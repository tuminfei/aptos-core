spec poto_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    ...
    /// published under the poto_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@poto_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@poto_framework);
    }
    ...
    spec initialize(poto_framework: &signer) {
        ...
        /// [high-level-req-3]
        aborts_if exists<StateStorageUsage>(@poto_framework);
        ensures exists<StateStorageUsage>(@poto_framework);
        let post state_usage = global<StateStorageUsage>(@poto_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@poto_framework).epoch;
    }
