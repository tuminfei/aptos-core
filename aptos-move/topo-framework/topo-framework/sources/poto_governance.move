...
module poto_framework::poto_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use poto_std::math64::min;
    use poto_std::simple_map::{Self, SimpleMap};
    use poto_std::smart_table::{Self, SmartTable};
    use poto_std::table::{Self, Table};

    use poto_framework::account::{Self, SignerCapability, create_signer_with_capability};
    use poto_framework::coin;
    use poto_framework::event::{Self, EventHandle};
    use poto_framework::governance_proposal::{Self, GovernanceProposal};
    use poto_framework::stake;
    use poto_framework::staking_config;
    use poto_framework::system_addresses;
    use poto_framework::poto_coin::{Self, TopoCoin};
    use poto_framework::chunky_dkg_config;
    use poto_framework::consensus_config;
    use poto_framework::permissioned_signer;
    use poto_framework::randomness_config;
    use poto_framework::reconfiguration_with_dkg;
    use poto_framework::timestamp;
    use poto_framework::voting;
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@poto_framework)) {
            move_to(
                poto_framework,
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@poto_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(poto_framework);
        initialize_partial_voting(poto_framework);
        move_to(poto_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(poto_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(poto_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(poto_framework),
            vote_events: account::new_event_handle<VoteEvent>(poto_framework),
        });
        move_to(poto_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(poto_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@poto_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(poto_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@poto_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@poto_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@poto_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@poto_framework);
    ...
    public fun get_remaining_voting_power(
        ...
        let used_voting_power = *VotingRecordsV2[@poto_framework].votes.borrow_with_default(record_key, &0);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@poto_framework);
    ...
    fun vote_internal(
        ...
        let used_voting_power = VotingRecordsV2[@poto_framework].votes.borrow_mut_with_default(record_key, 0);
        ...
        *used_voting_power += voting_power;
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@poto_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@poto_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@poto_framework, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<GovernanceProposal>(@poto_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposal>(@poto_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@poto_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@poto_framework).hashes;
    ...
    public entry fun reconfigure(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            ...
            reconfiguration_with_dkg::finish(poto_framework);
        }
    }
    ...
    public entry fun force_end_epoch(poto_framework: &signer) {
        system_addresses::assert_aptos_framework(poto_framework);
        reconfiguration_with_dkg::finish(poto_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(poto_framework: &signer) acquires GovernanceResponsbility {
        let core_signer = get_signer_testnet_only(poto_framework, @0x1);
    ...
    public fun toggle_features(poto_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_aptos_framework(poto_framework);
        features::change_feature_flags_for_next_epoch(poto_framework, enable, disable);
        reconfigure(poto_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(poto_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@poto_framework);
    ...
    fun assert_voting_initialization() {
        assert!(exists<VotingRecordsV2>(@poto_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_partial_voting(&poto_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(poto_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&poto_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@poto_framework).hashes;
        assert!(*approved_hashes.borrow(&0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @poto_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @poto_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@poto_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@poto_framework).hashes;
        assert!(!approved_hashes.contains_key(&0), 3);
    }

    #[test(poto_framework = @poto_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(poto_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(poto_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(poto_framework = @poto_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code = 0x5000a, location = poto_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(poto_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(poto_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            execution_hash.push_back(1);
            voting::resolve_proposal_v2<GovernanceProposal>(@poto_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@poto_framework, 0), 0);
            if (next_execution_hash.length() == 0) {
            ...
            voting::resolve<GovernanceProposal>(@poto_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@poto_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@poto_framework).hashes;
    ...
    #[test(poto_framework = @poto_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65541, location = poto_framework::poto_governance)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&poto_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(poto_framework = @poto_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65551, location = poto_framework::poto_governance)]
    public entry fun test_cannot_vote_for_expired_proposal(
        ...
        setup_partial_voting_with_initialized_stake(&poto_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(poto_framework = @poto_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @0x345)]
    #[expected_failure(abort_code = 65539, location = poto_framework::poto_governance)]
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&poto_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        ...
        test_resolving_proposal_generic(poto_framework, true, execution_hash);
    }
    ...
    public entry fun test_batch_vote(
        ...
        features::change_feature_flags_for_testing(&poto_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
        setup_partial_voting(&poto_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        let voter_1_addr = signer::address_of(&voter_1);
        let voter_2_addr = signer::address_of(&voter_2);
    ...
    public fun setup_voting(
        ...
        timestamp::set_time_has_started_for_testing(poto_framework);
        ...
        staking_config::initialize_for_test(poto_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(poto_framework, 10, 100, 1000);
        store_signer_cap(
            poto_framework,
            @poto_framework,
            account::create_test_signer_cap(@poto_framework),
        );
        ...
        stake::create_validator_set(poto_framework, active_validators, pks);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(poto_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(poto_framework, 10, 100, 1000);
        store_signer_cap(
            poto_framework,
            @poto_framework,
    ...
    public fun setup_partial_voting_with_initialized_stake(
        ...
        setup_voting_with_initialized_stake(poto_framework, proposer, yes_voter, no_voter);
    }
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&poto_framework));
        initialize(&poto_framework, 1, 2, 3);
        update_governance_config(&poto_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@poto_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = poto_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(poto_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
