...
module poto_framework::block {
    use std::error;
    use std::vector;
    use std::option;
    use poto_std::table_with_length::{Self, TableWithLength};
    use std::option::Option;
    use poto_framework::randomness;
    use poto_framework::decryption;
    ...
    public(friend) fun initialize(
        poto_framework: &signer, epoch_interval_microsecs: u64
    ) {
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                update_epoch_interval_events: account::new_event_handle<
                    UpdateEpochIntervalEvent>(poto_framework)
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        poto_framework: &signer, new_epoch_interval: u64
    ) acquires BlockResource {
        system_addresses::assert_aptos_framework(poto_framework);
        assert!(new_epoch_interval > 0, error::invalid_argument(EZERO_EPOCH_INTERVAL));

        let block_resource = borrow_global_mut<BlockResource>(@poto_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@poto_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@poto_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@poto_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@poto_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@poto_framework);
    ...
    public entry fun test_update_epoch_interval(poto_framework: signer) acquires BlockResource {
        account::create_account_for_test(@poto_framework);
        initialize(&poto_framework, 1);
        assert!(borrow_global<BlockResource>(@poto_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&poto_framework, 2);
        assert!(borrow_global<BlockResource>(@poto_framework).epoch_interval == 2, 1);
    }

    #[test(poto_framework = @poto_framework, account = @0x123)]
    ...
    public entry fun test_update_epoch_interval_unauthorized_should_fail(
        poto_framework: signer, account: signer
    ) acquires BlockResource {
        account::create_account_for_test(@poto_framework);
        initialize(&poto_framework, 1);
