...
module poto_framework::randomness {
    use std::hash;
    use std::option;
    ...
    public fun initialize(framework: &signer) {
        ...
        if (!exists<PerBlockRandomness>(@poto_framework)) {
            move_to(
                framework,
    ...
    public(friend) fun on_new_block(
        ...
        if (exists<PerBlockRandomness>(@poto_framework)) {
            let randomness = borrow_global_mut<PerBlockRandomness>(@poto_framework);
    ...
    fun next_32_bytes(): vector<u8> acquires PerBlockRandomness {
        ...
        let input = DST;
        let randomness = borrow_global<PerBlockRandomness>(@poto_framework);
    ...
    fun test_bytes(fx: signer) acquires PerBlockRandomness {
        ...
        let v = bytes(31);
        assert!(v.length() == 31, 0);
        let v = bytes(32);
        assert!(v.length() == 32, 0);

        let v = bytes(33);
        assert!(v.length() == 33, 0);
        let v = bytes(50);
        assert!(v.length() == 50, 0);
        let v = bytes(63);
        assert!(v.length() == 63, 0);
        let v = bytes(64);
        assert!(v.length() == 64, 0);
    }
