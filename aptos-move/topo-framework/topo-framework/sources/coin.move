...
module poto_framework::coin {
    ...
    use poto_framework::aggregator::Aggregator;
    use poto_framework::event::{Self, EventHandle};
    use poto_framework::guid;
    use poto_framework::optional_aggregator::{Self, OptionalAggregator};
    use poto_framework::permissioned_signer;
    use poto_framework::system_addresses;

    use poto_framework::fungible_asset::{
        Self,
        FungibleAsset,
        Metadata,
        MintRef,
    ...
    use poto_framework::create_signer;

    friend poto_framework::poto_coin;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@poto_framework)) {
            let map =
                &borrow_global<CoinConversionMap>(@poto_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(poto_framework: &signer) {
    ...
    inline fun is_topo<CoinType>(): bool {
        type_info::type_name<CoinType>() == string::utf8(b"0x1::poto_coin::TopoCoin")
    }
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(
        ...
        assert!(
            exists<CoinConversionMap>(@poto_framework),
            error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND)
        );
        let map = borrow_global_mut<CoinConversionMap>(@poto_framework);
        let type = type_info::type_of<CoinType>();
            ...
            let metadata_object_cref =
                if (is_topo) {
                    ...
                    object::create_named_object(
                        &create_signer::create_signer(@poto_fungible_asset),
    ...
    #[test(source = @0x2, framework = @poto_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = poto_framework::fungible_asset)]
    ...
    #[test(framework = @poto_framework, other = @0x123)]
    ...
    #[test(framework = @poto_framework)]
    #[expected_failure(abort_code = 0x20001, location = poto_framework::aggregator)]
    ...
    #[test(account = @poto_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @poto_framework, bob = @0xb0b)]
    ...
    #[test(account = @poto_framework)]
    #[expected_failure(abort_code = 0x50024, location = poto_framework::fungible_asset)]
