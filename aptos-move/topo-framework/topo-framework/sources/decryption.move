...
module poto_framework::decryption {
    use std::option;
    ...
    public fun initialize(framework: &signer) {
        ...
        if (!exists<PerBlockDecryptionKey>(@poto_framework)) {
            move_to(
                framework,
                PerBlockDecryptionKey { epoch: 0, round: 0, decryption_key: option::none() }
            );
        };
        if (!exists<PerEpochEncryptionKey>(@poto_framework)) {
            move_to(
                framework,
    ...
    public(friend) fun on_new_block(
        ...
        if (exists<PerBlockDecryptionKey>(@poto_framework)) {
            let decryption_key =
                borrow_global_mut<PerBlockDecryptionKey>(@poto_framework);
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires PerEpochEncryptionKey {
        ...
        if (config_buffer::does_exist<PerEpochEncryptionKey>()) {
            ...
            if (exists<PerEpochEncryptionKey>(@poto_framework)) {
                *borrow_global_mut<PerEpochEncryptionKey>(@poto_framework) = new_key;
            } else {
