...
module poto_framework::multisig_account {
    use poto_framework::account::{Self, SignerCapability, new_event_handle, create_resource_address};
    ...
    use poto_framework::timestamp::now_seconds;
    ...
    /// account.
    const DOMAIN_SEPARATOR: vector<u8> = b"poto_framework::multisig_account";
    ...
    #[test_only]
    use poto_framework::coin::{destroy_mint_cap, destroy_burn_cap};
    ...
    fun setup() {
        ...
        let (burn, mint) = poto_coin::initialize_for_test(framework_signer);
    ...
    public entry fun test_update_metadata(owner: &signer) acquires MultisigAccount {
        ...
        assert!(updated_metadata.borrow(&utf8(b"key1")) == &vector[1], 0);
        assert!(updated_metadata.borrow(&utf8(b"key2")) == &vector[2], 0);
    }
    ...
    public entry fun test_execute_transaction_successful(
        ...
        approve_transaction(owner_2, multisig_account, 1);
        assert!(can_be_executed(multisig_account, 1), 1);
        assert!(borrow_global<MultisigAccount>(multisig_account).transactions.contains(1), 0);
