...
module poto_framework::poto_coin {
    use std::error;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    use poto_framework::coin::{Self, BurnCapability, MintCapability};
    ...
    public(friend) fun initialize(poto_framework: &signer): (BurnCapability<TopoCoin>, MintCapability<TopoCoin>) {
        ...
        move_to(poto_framework, MintCapStore { mint_cap });
    ...
    public(friend) fun destroy_mint_cap(poto_framework: &signer) acquires MintCapStore {
        system_addresses::assert_aptos_framework(poto_framework);
        let MintCapStore { mint_cap } = move_from<MintCapStore>(@poto_framework);
    ...
    #[test_only]
    use poto_framework::fungible_asset::FungibleAsset;
    ...
    public fun mint_topo_fa_for_test(amount: u64): FungibleAsset acquires MintCapStore {
        ...
        coin::coin_to_fungible_asset(
            coin::mint(
                amount,
                &borrow_global<MintCapStore>(@poto_framework).mint_cap
            )
        )
    }
    ...
    public fun ensure_initialized_with_topo_fa_metadata_for_test() {
        let poto_framework = account::create_signer_for_test(@poto_framework);
        if (!exists<MintCapStore>(@poto_framework)) {
            ...
            let (burn_cap, mint_cap) = initialize(&poto_framework);
    ...
    #[test_only]
    public fun initialize_for_test(poto_framework: &signer): (BurnCapability<TopoCoin>, MintCapability<TopoCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(poto_framework);
        let (burn_cap, mint_cap) = initialize(poto_framework);
        coin::create_coin_conversion_map(poto_framework);
        coin::create_pairing<TopoCoin>(poto_framework);
