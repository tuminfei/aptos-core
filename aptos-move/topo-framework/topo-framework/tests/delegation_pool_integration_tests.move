#[test_only]
module poto_framework::delegation_pool_integration_tests {
    use std::features;
    use std::signer;
    ...
    use poto_framework::delegation_pool as dp;
    ...
    #[expected_failure(abort_code = 0x10007, location = poto_framework::stake)]
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_TOPO, false, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator, 9900 * ONE_TOPO + 1);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator_2, 9900 * ONE_TOPO + 1);
    }
    ...
    public entry fun test_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_TOPO, true, true);
    ...
    public entry fun test_active_validator_with_pending_inactive_stake_cannot_add_stake_if_exceeding_max_allowed(
        ...
        dp::unlock(validator, pool_address, 50 * ONE_TOPO);
        stake::assert_validator_state(pool_address, 50 * ONE_TOPO, 0, 0, 50 * ONE_TOPO, 0);

        // Add 9900 TOPO + 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 TOPO + 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator, 9900 * ONE_TOPO + 1);
    }
    ...
    public entry fun test_pending_inactive_cannot_add_stake_if_exceeding_max_allowed(
        ...
        // Add 9900 TOPO + 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 TOPO + 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator_1, 9900 * ONE_TOPO + 1);
    }
    ...
    public entry fun test_end_to_end(
        ...
        stake::mint(validator, 900 * ONE_TOPO);
        dp::add_stake(validator, pool_address, 100 * ONE_TOPO);
        assert!(
            coin::balance<TopoCoin>(validator_address) == 800 * ONE_TOPO,
            2
        );
        stake::assert_validator_state(pool_address, 100 * ONE_TOPO, 0, 100 * ONE_TOPO, 0, 0);
        ...
        stake::assert_validator_state(pool_address, 201 * ONE_TOPO, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_TOPO);
        ...
        dp::withdraw(validator, pool_address, 50 * ONE_TOPO);
        ...
        dp::withdraw(validator, pool_address, 51 * ONE_TOPO);
    ...
    #[test(poto_framework = @poto_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x1000D, location = poto_framework::stake)]
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        mint_and_add_stake(validator_2, 100 * ONE_TOPO);
        stake::assert_validator_state(validator_2_address, 200 * ONE_TOPO, 0, 0, 0, 0);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator_2, ONE_TOPO);
    }
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_TOPO, true, false);
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit_in_multiple_epochs(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(validator_address, 100 * ONE_TOPO, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 110 * ONE_TOPO, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 121 * ONE_TOPO, 0, 0, 0, 0);
        // Add more than 50% limit. The following line should fail.
        mint_and_add_stake(validator, 99 * ONE_TOPO);
    }
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator, 50 * ONE_TOPO + 1);
    }
    ...
    public entry fun test_active_validator_unlock_partial_stake(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_TOPO);
    ...
    public entry fun test_active_validator_can_withdraw_all_stake_and_rewards_at_once(
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_TOPO, 0, 0, 0, 1);
        ...
        dp::unlock(validator, validator_address, 101 * ONE_TOPO);
        stake::assert_validator_state(validator_address, 0, 0, 0, 101 * ONE_TOPO, 1);
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x10006, location = poto_framework::delegation_pool)]
    public entry fun test_active_validator_unlocking_more_than_available_stake_should_cap(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 200 * ONE_TOPO);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 100 * ONE_TOPO);
        ...
        dp::withdraw(validator, validator_address, 200 * ONE_TOPO);
    ...
    public entry fun test_active_validator_can_reactivate_pending_inactive_stake(
        ...
        dp::reactivate_stake(validator, validator_address, 50 * ONE_TOPO);
        stake::assert_validator_state(validator_address, 100 * ONE_TOPO, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_reactivate_more_than_available_pending_inactive_stake_should_cap(
        ...
        dp::reactivate_stake(validator, validator_address, 51 * ONE_TOPO);
        stake::assert_validator_state(validator_address, 100 * ONE_TOPO, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_leaves_staking_but_still_has_a_lockup(
        ...
        stake::assert_validator_state(validator_address, 100 * ONE_TOPO, 0, 0, 0, 1);
        end_epoch();
    ...
    public entry fun test_pending_inactive_validator_does_not_count_in_increase_limit(
        ...
        mint_and_add_stake(validator_1, 51 * ONE_TOPO);
    }
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        end_epoch();
        assert!(
            stake::get_validator_state(validator_1_address) == VALIDATOR_STATUS_ACTIVE,
            0
        );
        assert!(
            stake::get_validator_state(validator_2_address) == VALIDATOR_STATUS_ACTIVE,
            1
        );

        // Validator indices is the reverse order of the joining order.
        stake::assert_validator_state(validator_1_address, 100 * ONE_TOPO, 0, 0, 0, 0);
        stake::assert_validator_state(validator_2_address, 100 * ONE_TOPO, 0, 0, 0, 1);
        ...
        stake::assert_validator_state(validator_1_address, 101 * ONE_TOPO, 0, 0, 0, 0);
        assert!(
            stake::get_validator_state(validator_2_address)
        ...
        stake::assert_validator_state(validator_2_address, 101 * ONE_TOPO, 0, 0, 0, 1);
        assert!(
            stake::get_validator_state(validator_3_address) == VALIDATOR_STATUS_ACTIVE,
            10
        );
        stake::assert_validator_state(validator_3_address, 100 * ONE_TOPO, 0, 0, 0, 1);

        // Validators without enough stake will be removed.
        dp::unlock(validator_1, validator_1_address, 50 * ONE_TOPO);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();
        assert!(
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        mint_and_add_stake(validator, 100 * ONE_TOPO);
        let pool_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(pool_address, 100 * ONE_TOPO, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_TOPO);
        stake::assert_validator_state(pool_address, 0, 0, 0, 100 * ONE_TOPO, 0);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();

        // Withdraw stake + rewards.
        stake::assert_validator_state(pool_address, 0, 101 * ONE_TOPO, 0, 0, 0);
        dp::withdraw(validator, pool_address, 101 * ONE_TOPO);
        assert!(
            coin::balance<TopoCoin>(signer::address_of(validator)) == 101 * ONE_TOPO,
            1
        );
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000A, location = poto_framework::stake)]
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000E, location = poto_framework::stake)]
    ...
    #[test(poto_framework = @poto_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000B, location = poto_framework::stake)]
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        let validator_to_remove =
            dp::get_owned_pool_address(signer::address_of(validator_1));
        stake::remove_validators(poto_framework, &vector[validator_to_remove]);
        assert!(
