...
- `src/head.mrb` — BCS-serialized `ReleaseBundle` containing all compiled Move framework packages (move-stdlib, topo-stdlib, topo-framework, topo-token, topo-token-objects, topo-trading, topo-experimental).
- `src/topo_framework_sdk_builder.rs` — Rust SDK bindings for topo-framework.
- `src/topo_token_sdk_builder.rs` — Rust SDK bindings for topo-token.
- `src/topo_token_objects_sdk_builder.rs` — Rust SDK bindings for topo-token-objects.
...
- Move source files under `aptos-move/framework/` (e.g. in `topo-framework/sources/`, `topo-stdlib/sources/`, `move-stdlib/sources/`, etc.)
