...
/// Create a new collection
pub fn poto_token_create_collection(
    description: Vec<u8>,
    max_supply: u64,
    name: Vec<u8>,
    uri: Vec<u8>,
...
/// With an existing collection, directly mint a soul bound token into the recipient's account.
pub fn poto_token_mint_soul_bound(
    collection: Vec<u8>,
    description: Vec<u8>,
    name: Vec<u8>,
    uri: Vec<u8>,
...
mod decoder {
    ...
    pub fn poto_token_mint(payload: &TransactionPayload) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
    ...
    pub fn poto_token_mint_soul_bound(payload: &TransactionPayload) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
...
static SCRIPT_FUNCTION_DECODER_MAP: once_cell::sync::Lazy<EntryFunctionDecoderMap> =
    once_cell::sync::Lazy::new(|| {
        ...
        map.insert(
            "poto_token_create_collection".to_string(),
            Box::new(decoder::poto_token_create_collection),
        );
        map.insert(
            "poto_token_mint".to_string(),
            Box::new(decoder::poto_token_mint),
        );
        map.insert(
            "poto_token_mint_soul_bound".to_string(),
            Box::new(decoder::poto_token_mint_soul_bound),
        );
        map
    });
