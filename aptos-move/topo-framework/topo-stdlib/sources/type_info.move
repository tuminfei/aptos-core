module poto_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `poto_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `poto_framework::chain_id::get()` will
    /// return whichever ID was passed to `poto_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::poto_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(type_info.account_address() == @poto_std, 0);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @poto_std;
