...
impl ReleaseTarget {
    ...
    pub fn packages(self) -> Vec<(&'static str, Option<&'static str>, bool)> {
        let result = vec![
            ...
            (
                "topo-framework",
                Some("cached-packages/src/topo_framework_sdk_builder.rs"),
                false,
            ),
            (
                "topo-token",
            ...
            (
                "topo-token-objects",
                Some("cached-packages/src/poto_token_objects_sdk_builder.rs"),
                false,
            ),
            ("topo-trading", None, false),
            ("topo-experimental", None, true),
        ];
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    result.insert("poto_std".to_owned(), one);
    result.insert("poto_framework".to_owned(), one);
    result.insert("poto_token".to_owned(), three);
    result.insert("poto_token_objects".to_owned(), four);
    result.insert("poto_trading".to_owned(), five);
    result.insert("poto_experimental".to_owned(), seven);
    result.insert("poto_fungible_asset".to_owned(), ten);
