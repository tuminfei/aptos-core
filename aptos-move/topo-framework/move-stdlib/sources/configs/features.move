...
module std::features {
    ...
    /// Whether the new `poto_stdlib::type_info::chain_id()` native for fetching the chain ID is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    const TOPOOS_STD_CHAIN_ID_NATIVES: u64 = 4;

    public fun get_aptos_stdlib_chain_id_feature(): u64 {
        TOPOOS_STD_CHAIN_ID_NATIVES
    }

    public fun poto_stdlib_chain_id_enabled(): bool {
        is_enabled(TOPOOS_STD_CHAIN_ID_NATIVES)
    }
    ...
    /// Whether the new `poto_stdlib::multi_ed25519::public_key_validate_internal_v2()` native is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    ...
    /// Lifetime: transient
    const TOPOOS_UNIQUE_IDENTIFIERS: u64 = 23;
    ...
    /// Deprecated by `poto_framework::randomness_config::RandomnessConfig`.
    ...
    /// Deprecated by `poto_framework::jwk_consensus_config::JWKConsensusConfig`.
    ...
    const PRIMARY_TOPO_FUNGIBLE_STORE_AT_USER_ADDRESS: u64 = 61;

    #[deprecated]
    public fun get_primary_topo_fungible_store_at_user_address_feature(): u64 {
        abort error::invalid_argument(EINVALID_FEATURE)
    }

    #[deprecated]
    public fun primary_topo_fungible_store_at_user_address_enabled(): bool {
        true
    }
    ...
    /// Lifetime: transient
    const NEW_ACCOUNTS_DEFAULT_TO_FA_TOPO_STORE: u64 = 64;

    public fun get_new_accounts_default_to_fa_topo_store_feature(): u64 {
        NEW_ACCOUNTS_DEFAULT_TO_FA_TOPO_STORE
    }

    public fun new_accounts_default_to_fa_topo_store_enabled(): bool {
        is_enabled(NEW_ACCOUNTS_DEFAULT_TO_FA_TOPO_STORE)
    }

    /// Lifetime: transient
    const OPERATIONS_DEFAULT_TO_FA_TOPO_STORE: u64 = 65;

    #[deprecated]
    public fun get_operations_default_to_fa_topo_store_feature(): u64 {
        abort error::invalid_argument(EINVALID_FEATURE)
    }

    #[deprecated]
    public fun operations_default_to_fa_topo_store_enabled(): bool {
        true
    }
