...
fn execute_cross_module_enum_new_variant_handling() {
    ...
    let mut types_builder = PackageBuilder::new("types");
    types_builder.add_source(
        "types.move",
        r#"
        module 0x815::types {
    ...
    let types_path = types_builder.write_to_temp().unwrap();
    ...
    ops_builder.add_local_dep("types", types_path.path().to_str().unwrap());
    let ops_path = ops_builder.write_to_temp().unwrap();
    let status = h.publish_package_with_options(
        &acc2,
        ops_path.path(),
        BuildOptions::move_2().set_latest_language(),
    ...
    let upgraded_path = upgraded_builder.write_to_temp().unwrap();
    let status = h.publish_package_with_options(
        &acc,
        upgraded_path.path(),
        BuildOptions::move_2().set_latest_language(),
