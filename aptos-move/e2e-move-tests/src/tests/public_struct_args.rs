...
fn test_option_u64_vector_at_limit() {
    ...
    // Create a vector with exactly 32 Option<u64> elements (Some(1), ..., Some(32))
    // PUBLIC_STRUCT_ENUM_ARGS is enabled by default, so max_invocations = 32.
    let mut options = vec![];
    for i in 1..=32 {
        let some_value = MoveValue::Vector(vec![MoveValue::U64(i)]);
        options.push(some_value);
    ...
    let result = get_test_result(&h, acc.address());
    // Expected sum: 1 + 2 + 3 + ... + 32 = 32 * 33 / 2 = 528
...
/// Test that vector of 33 Option<u64> elements is rejected with the new limit.
/// With PUBLIC_STRUCT_ENUM_ARGS enabled, max_invocations is 32; 33 elements exceeds it.
#[test]
...
/// Test vector<Option<Point>> with exactly 16 Some(Point) elements.
/// Each Some(Point) costs 2 invocations (1 for Option + 1 for Point).
/// 16 × 2 = 32 invocations, exactly at the limit. Expect success.
#[test]
fn test_vector_option_struct_at_limit() {
    ...
    // 16 × Some(Point{i, i}) — 16 × 2 = 32 invocations (exactly at the limit)
    ...
    let result = get_test_result(&h, acc.address());
    // sum of 2*i for i in 0..6 = 2 * (0+1+...+15) = 2 * 120 = 240
...
/// Test vector<Option<Point>> with 17 Some(Point) elements, which exceeds the limit.
/// 17 × 2 = 34 invocations > 32. Expect failure.
#[test]
fn test_vector_option_struct_exceeds_limit() {
    ...
    // 17 × Some(Point{1,1}) — 17 × 2 = 34 invocations > 32 limit
    let opts = MoveValue::Vector(
        (0..17)
            .map(|_| {
                MoveValue::Vector(vec![MoveValue::Struct(MoveStruct::Runtime(vec![
...
/// Test vector<vector<Point>> — exercises an additional nesting level in both the BCS encoder
/// and the pack invocation counter (gap 1).
///
/// All existing vector tests use `vector<Point>` or `vector<Option<u64>>`.  A
/// `vector<vector<Point>>` nests the pack calls one level deeper.
#[test]
fn test_vector_of_vector_of_structs() {
    let mut h = setup_harness();
    let acc = h.new_account_at(AccountAddress::from_hex_literal("0xcafe").unwrap());
...
/// Test Pair<SimpleTag, PublicPoint>: one type arg is an enum, the other is a struct (gap 2).
///
/// The existing `test_pair_both_type_params_valid` only uses (PublicPoint, PublicPoint).
/// This test exercises the mixed case: first = public enum, second = public struct.
#[test]
fn test_pair_mixed_enum_and_struct_type_params() {
    let mut h = setup_harness();
    let acc = h.new_account_at(AccountAddress::from_hex_literal("0xcafe").unwrap());
    ...
    // Pair<SimpleTag, PublicPoint> { first: SimpleTag::Y (variant index 1), second: {x:3, y:7} }
    let simple_tag_y = MoveValue::Struct(MoveStruct::RuntimeVariant(1, vec![])); // SimpleTag::Y
...
/// Test a struct with a user-defined enum field: Labeled { color: Color, value: u64 } (gap 4).
///
/// All existing tests use flat structs or structs containing other structs.  This test passes
/// a struct whose field is a user-defined enum (Color) as a top-level entry argument.
#[test]
fn test_struct_with_enum_field() {
    ...
    // Labeled { color: Color::Green (variant index 1, no fields), value: 10 }
    // tag for Green = 2; result.value = 2 + 10 = 12
    let color_green = MoveValue::Struct(MoveStruct::RuntimeVariant(1, vec![])); // Color::Green
    ...
    let result = get_test_result(&h, acc.address());
    assert_eq!(result.value, 12); // Green tag (2) + value (10)
    assert_eq!(
        String::from_utf8(result.message).unwrap(),
        "labeled_received"
    );
...
/// Test that passing a public struct while PUBLIC_STRUCT_ENUM_ARGS is disabled is rejected (gap 5).
///
/// No existing test explicitly submits a public-struct argument with the feature off and verifies
/// the resulting error.  The feature gate sits in `is_public_struct_arg`; when it returns false
/// the validator falls through to `invalid_signature()`.
#[test]
fn test_public_struct_rejected_when_feature_flag_disabled() {
    let mut h = setup_harness();
    let acc = h.new_account_at(AccountAddress::from_hex_literal("0xcafe").unwrap());
    // Disable PUBLIC_STRUCT_ENUM_ARGS so public structs are not accepted as transaction args.
    h.enable_features(vec![], vec![FeatureFlag::PUBLIC_STRUCT_ENUM_ARGS]);
    ...
    // Point { x: 1, y: 2 } — valid argument when feature is on, but rejected when off.
    let point = MoveValue::Struct(MoveStruct::Runtime(vec![
        MoveValue::U64(1),
        MoveValue::U64(2),
    ]));
    ...
    // The validator cannot find a valid argument type for Point when the feature is off.
    assert_vm_status!(status, StatusCode::INVALID_MAIN_FUNCTION_SIGNATURE);
}
