...
/// (2) when exchanging coins, we call `exchange_to` to swap `TopoCoin` to `ChloesCoin`, and `exchange_from` to swap `TopoCoin` from `ChloesCoin`
module resource_account::simple_defi {
    ...
    /// Exchange TopoCoin to ChloesCoin
    public fun exchange_to(a_coin: Coin<TopoCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    /// Exchange ChloesCoin to TopoCoin
    public fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<TopoCoin> acquires ModuleData {
        ...
        coin::withdraw<TopoCoin>(&resource_signer, amount)
    }
    ...
    public entry fun exchange_to_entry(account: &signer, amount: u64) acquires ModuleData {
        let a_coin = coin::withdraw<TopoCoin>(account, amount);
    ...
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let five_a_coins = coin::mint(5, &topo_coin_mint_cap);
        let c_coins = exchange_to(five_a_coins);
        assert!(coin::value(&c_coins) == 5, 0);
        assert!(coin::balance<TopoCoin>(signer::address_of(&resource_account)) == 5, 1);
        ...
        assert!(coin::balance<TopoCoin>(signer::address_of(&resource_account)) == 0, 3);
        ...
        coin::burn(a_coins, &topo_coin_burn_cap);
        coin::destroy_mint_cap<TopoCoin>(topo_coin_mint_cap);
        coin::destroy_burn_cap<TopoCoin>(topo_coin_burn_cap);
    }
}
