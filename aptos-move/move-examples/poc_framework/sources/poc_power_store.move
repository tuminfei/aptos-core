...
module poc_framework::poc_power_store {
    use std::error;
    use std::signer;
    ...
    /// 更新唯一 operator 地址。
    /// 仅限 @poc_framework 调用；新旧地址相同则幂等返回。
    public entry fun set_operator(
        ...
        let store = borrow_global_mut<PowerStore>(@poc_framework);
    ...
    public fun get_user_power_info(user: address): UserPowerInfo acquires PowerStore {
        ...
        let store = borrow_global<PowerStore>(@poc_framework);
    ...
    public fun get_operator(): address acquires PowerStore {
        ...
        borrow_global<PowerStore>(@poc_framework).operator
    }
    ...
    fun assert_store_exists() {
        assert!(
            exists<PowerStore>(@poc_framework),
    ...
    #[test(framework = @poc_framework, operator = @0xA, user1 = @0xB, user2 = @0xC)]
    ...
    #[test(framework = @poc_framework, operator = @0xA, user1 = @0xB)]
