...
module poc_framework::poc_registry {
    use std::error;
    ...
    // 全局注册中心，存储在 @poc_framework 地址下，genesis 时初始化。
    ...
    // 初始化注册中心资源。
    // 仅限 poc_framework 地址调用，且幂等——如果已存在则跳过。
    public entry fun initialize_registry(poc_framework: &signer) {
        //system_addresses::assert_aptos_framework(poc_framework);
        if (!exists<Registry>(@poc_framework)) {
            move_to(poc_framework, Registry {
                apps: table::new(),
    ...
    public entry fun register_app(
        ...
        assert!(
            exists<Registry>(@poc_framework),
            error::not_found(EREGISTRY_NOT_INITIALIZED),
        );
        let registry = borrow_global_mut<Registry>(@poc_framework);
    ...
    // 挂起期间应用的贡献事件不计入算力。
    // 仅限 poc_framework 地址调用。
    public entry fun suspend_poc_listing(
        poc_framework: &signer,
        app_admin: address,
    ) acquires Registry {
        set_poc_listing_status(poc_framework, app_admin, POC_LISTING_STATUS_SUSPENDED);
    }
    ...
    public entry fun whitelist_app_for_poc(
        ...
        set_poc_listing_status(poc_framework, app_admin, POC_LISTING_STATUS_WHITELISTED);
    }
    ...
    public fun exists_app(app_admin: address): bool acquires Registry {
        ...
        borrow_global<Registry>(@poc_framework).apps.contains(app_admin)
    }
    ...
    public fun resolve_app_admin_by_app_address(
        ...
        let registry = borrow_global<Registry>(@poc_framework);
