module topo_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `topo_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `topo_framework::chain_id::get()` will
    /// return whichever ID was passed to `topo_framework::chain_id::initialize_for_test()`.
    ...
    /// Example: 0x1::coin::CoinStore<0x1::topo_coin::TopoCoin>
    ...
    fun test_type_of() {
        ...
        assert!(type_info.account_address() == @topo_std, 0);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @topo_std;
