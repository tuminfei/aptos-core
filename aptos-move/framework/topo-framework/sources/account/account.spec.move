spec topo_framework::account {
    ...
    /// to the OriginatingAddress under the topo_framework account.
    ...
    /// Only the address `@topo_framework` can call.
    /// OriginatingAddress does not exist under `@topo_framework` before the call.
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @topo_framework || new_address == @aptos_token;
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@topo_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@topo_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@topo_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@topo_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@topo_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@topo_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
