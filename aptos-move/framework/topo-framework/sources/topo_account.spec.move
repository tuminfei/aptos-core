spec topo_framework::topo_account {
    /// <high-level-req>
    /// No.: 1
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @topo_framework || auth_key == @aptos_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        /// [high-level-req-5]
        ensures exists<topo_framework::account::Account>(to);
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @topo_framework || recipients[i] == @aptos_token);
    ...
    spec deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) {
        ...
        /// [high-level-spec-6]
        ensures exists<topo_framework::account::Account>(to);
        ensures exists<topo_framework::coin::CoinStore<CoinType>>(to);
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @topo_framework || to == @aptos_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<topo_framework::account::Account>(to);
