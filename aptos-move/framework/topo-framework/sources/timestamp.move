...
module topo_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(topo_framework: &signer) {
        system_addresses::assert_aptos_framework(topo_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(topo_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@topo_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@topo_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@topo_framework).microseconds
    }
