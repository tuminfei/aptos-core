spec topo_framework::block {
    ...
    /// Implementation: The initialize and  update_epoch_interval_microsecs functions ensure that only topo_framework
    /// can call them.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@topo_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@topo_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@topo_framework).new_block_events) == 0;
    ...
    spec initialize(topo_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@topo_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@topo_framework);
        aborts_if !exists<reconfiguration::Configuration>(@topo_framework);
