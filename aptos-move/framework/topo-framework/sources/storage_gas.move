...
module topo_framework::storage_gas {
    ...
    public(friend) fun set_config(topo_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_aptos_framework(topo_framework);
        *borrow_global_mut<StorageGasConfig>(@topo_framework) = config;
    }
    ...
    public fun initialize(topo_framework: &signer) {
        system_addresses::assert_aptos_framework(topo_framework);
        assert!(
            !exists<StorageGasConfig>(@topo_framework),
        ...
        move_to(topo_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@topo_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(topo_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@topo_framework);
        let gas = borrow_global_mut<StorageGas>(@topo_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@topo_framework);
