spec aptos_framework::stake {
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<TopoCoinCapabilities>(@aptos_framework);
    ...
    )
    {
        ...
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<TopoCoin>>(addr) ==>
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<TopoCoin> { amount: amount };
    }
